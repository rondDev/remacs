...
- A custom filename modifier is available in Rmcs: ~:P~, which expands to the
...
| vim-unimpaired        | (provided by Rmcs)             | [[https://github.com/hlissner/rmcs-emacs/blob/develop/modules/editor/evil/config.el#L413-L460][see the list]]                   |
...
Rmcs replaces the [[kbd:][s]] and [[kbd:][S]] keys with the [[rmcs-package:evil-snipe]] package (a port of
...
Rmcs changes the behavior of the [[kbd:][Y]] key in normal mode to yank-to-EOL (equivalent
