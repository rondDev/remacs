...
=$RMCSDIR/snippets/{major-mode}/{snippet-name}=. The yasnippet documentation
...
Add snippet files to =$RMCSDIR/snippets/text-mode/= with the =__licenses-=
prefix and [[fn:+file-templates/insert-license]] will recognize them. E.g.
=$RMCSDIR/snippets/text-mode/__license-mit=.
