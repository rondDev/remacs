...
(defun +rss-cleanup-h ()
  ...
  (let ((buf (previous-buffer)))
    (when (or (null buf) (not (rmcs-real-buffer-p buf)))
      (switch-to-buffer (rmcs-fallback-buffer))))
  (let ((search-buffers (rmcs-buffers-in-mode 'elfeed-search-mode))
        (show-buffers (rmcs-buffers-in-mode 'elfeed-show-mode))
