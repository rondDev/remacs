...
*** Easymotion
If you use [[rmcs-package:evil-easymotion]], then all the bindings that were on [[kbd:][gs]] have been
moved to [[kbd:][gé]].
...
*** Org-mode
[[rmcs-package:evil-org]] allows to define ~evil-org-movement-bindings~ to automatically map
...
*** Bépo
- In [[rmcs-package:eshell]], the key [[kbd:][c]] is still bound to ~evil-collection-eshell-evil-change~ in
  normal mode.
...
"get work done". Sadly the only way is to comment out the module, run ~$ rmcs
