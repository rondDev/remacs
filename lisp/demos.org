...
are great, but this file exists to add demos for Rmcs's API and beyond.

#+begin_quote
 󰐃 Please make sure new additions to this file are arranged alphabetically and
    has a :PROPERTIES: drawer that includes in what version of Rmcs that the
    symbol/function was added.
#+end_quote

#+begin_quote
 󰐃 Please don't add demos for code outside of Rmcs Emacs. PR those to the parent
...
;; You may utilise `rmcs-themes's theme API to fetch or tweak colors from their
;; palettes. No need to wait until the theme or package is loaded. e.g.
(custom-set-faces!
 `(outline-1 :foreground ,(rmcs-color 'red))
 `(outline-2 :background ,(rmcs-color 'blue)))
#+end_src
...
:END:
#+begin_src emacs-lisp :eval no
(custom-theme-set-faces! 'rmcs-one
 '(outline-1 :weight normal)
 '(outline-2 :weight normal)
 '(outline-3 :weight normal)
 '(outline-4 :weight normal)
...
(custom-theme-set-faces! '(rmcs-one-theme rmcs-one-light-theme)
 '((outline-1 outline-2 outline-3 outline-4 outline-5 outline-6)
   :weight normal)
 '((default region)
   :background "red" :weight bold))
...
:END:
#+begin_src emacs-lisp :eval no
;; Disable packages enabled by RMCS
...
(rmcs!
 ...
 (source "modules/")  ; Modules in $RMCSDIR/modules/*/*
 (source :name rmcs :repo "rmcsemacs/modules" :pin "v21.12.0")  ; Rmcs's default modules
 (source :name contrib :repo "rmcsemacs/contrib-modules" :pin "v21.12.0")  ; Community-contributed
 ;; Examples:
 ...
 :lang
 (rust   :repo "rmcsemacs/modules" :branch "somePR" :pin "1a2b3c4d"
         :path "lang/rust"
         :flags '(-lsp))
 ;; A local, out-of-tree module
 (python :path "/nonstandard/location/for/modules/python"
         :flags '(+pyenv +conda))
 (cc     :source 'rmcs)         ; explicit source
 (agda   :source '(rmcs more))  ; multiple sources
...
:END:
#+begin_src emacs-lisp
(file-exists-p! "init.el" rmcs-emacs-dir)
#+end_src
...
(seq-sort (fn! (string-lessp (symbol-name %1)
                             (symbol-name %2)))
          '(bonzo foo bar buddy rmcsguy baz zombies))
#+end_src
...
:END:
#+begin_src emacs-lisp :eval no
;;; Lets say we're in ~/.rmcs.d/config.el
(load! "lisp/module")                  ; loads ~/.rmcs.d/lisp/module.el
(load! "somefile" rmcs-emacs-dir)      ; loads ~/.emacs.d/somefile.el
(load! "anotherfile" rmcs-user-dir)    ; loads ~/.rmcs.d/anotherfile.el
...
;; To disable a package included with Rmcs (which will no-op all its `after!'
...
;; If you share your config between two computers, and don't want bin/rmcs
