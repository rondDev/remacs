...
(defvar rmcs-ci-commit-trailer-keys
  '(("Fix" ref hash url)
    ("Ref" ref hash url)
    ("Close" ref)
    ("Revert" ref hash)
...
(defvar rmcs-ci-commit-trailer-types
  '((ref      . "^\\(https?://[^ ]+\\|[^/]+/[^/]+\\)?#[0-9]+$")
...
- A function predicate, taking two arguments (a scope as a symbol, and a plist
  containing information about the current commit--see `rmcs-ci-commit-scopes'
...
(defvar rmcs-ci-commit-rules
  ...
  (list (lambda! (&key subject)
        ...
        (lambda! (&key type)
          ...
          (or (memq type rmcs-ci-commit-types)
              (if type
          ...
          (dolist (scope scopes)
            (condition-case e
                (letf! (defun* check-rule (rule)
                  ...
                  (or (seq-find #'check-rule rmcs-ci-commit-scopes)
        ...
        (lambda! (&key body trailers)
          ...
          (let* ((keys   (mapcar #'car rmcs-ci-commit-trailer-keys))
            ...
            (pcase-dolist (`(,key . ,value) trailers)
              (if (and (not (memq 'name (cdr (assoc key rmcs-ci-commit-trailer-keys))))
                       (string-match-p " " value))
                  (fail! "Found %S, but only one value allowed per trailer"
                         (truncate-string-to-width (concat key ": " value) 20 nil nil "…"))
                (when-let (allowed-types (cdr (assoc key rmcs-ci-commit-trailer-keys)))
                  (or (cl-loop for type in allowed-types
                               if (cdr (assq type rmcs-ci-commit-trailer-types))
...
(cl-defun rmcs-ci-enforce-scopeless-types (scope (&key type scopes summary &allow-other-keys))
  "Complain about scoped commit types that shouldn't be scoped."
  (when (memq type rmcs-ci-commit-scopeless-types)
    (user-error "Scopes for %s commits should go after the colon, not before"
                type)))


(defun rmcs-ci--parse-commit (commit-msg)
  (with-temp-buffer
...
(defun rmcs-ci--parse-bumps (from end)
  (with-temp-buffer
    (save-excursion
      (insert
       (cdr (rmcs-call-process "git" "log" "--format=full" "--grep=\\(bump\\|revert\\):"
                               (format "%s...%s" from end)))))
    (save-match-data
      (let (packages)
  ...
  (let ((warnings 0)
    ...
    (let ((issues (+ warnings failures)))
      (if (= issues 0)
        ...
        (print! "\nSee https://discourse.rmcsemacs.org/git-conventions")
