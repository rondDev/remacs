...
;; When __RMCSDUMP is set, rmcsscripts trigger this special handler.
(defcli! (:root :dump)
  ...
  (let* ((prefix (rmcs-cli-context-prefix context))
    ...
    (funcall (if pretty? #'pp #'prin1)
             (cond ((equal commands '("-")) (hash-table-values rmcs-cli--table))
                   (commands (rmcs-cli-find command))
                   ((rmcs-cli-find (list prefix)))))
    (terpri)
...
(defun rmcs-cli-help--similarity (a b)
  (- 1 (/ (float (rmcs-cli-help--string-distance a b))
          (max (length a) (length b)))))

(defun rmcs-cli-help--string-distance (a b)
