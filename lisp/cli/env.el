...
;; (defvar rmcs-env-file
;;   (rmcs-path rmcs-profile-dir
;;              rmcs-profile-init-dir-name
;;              "10-init-env.el")
...
(defvar rmcs-env-deny
  '(;; Unix/shell state that shouldn't be persisted
    ...
    "^GPG_AGENT_INFO$"
    ;; Internal Rmcs envvars
    "^DEBUG$" "^INSECURE$" "^\\(EMACS\\|RMCS\\)DIR$"
    "^RMCS\\(PATH\\|PROFILE\\)$" "^__")
  "Environment variables to omit from envvar files.

Each string is a regexp, matched against variable names to omit from
`rmcs-env-file'.")

(defvar rmcs-env-allow '()
  "Environment variables to include in envvar files.

This overrules `rmcs-env-deny'. Each string is a regexp, matched against
variable names to omit from `rmcs-env-file'.")
...
'$SHELL -c set' on windows). Rmcs loads this file at startup (if it exists) to
...
This file is automatically regenerated when you run this command or 'rmcs sync'.
However, 'rmcs sync' will only regenerate this file if it exists.
...
Why this over exec-path-from-shell?
  ...
  1. `exec-path-from-shell' spawns (at least) one process at startup to scrape
     ...
     framework (like oh-my-zsh) could undo all of Rmcs's startup optimizations
     in one fell swoop.
  ...
  (let ((env-file (rmcs-path (or output-file rmcs-env-file))))
    (with-temp-file env-file
      ...
      (print-group!
       ...
       (insert
        ...
        ";; ---------------------------------------------------------------------------\n"
        ";; This file was auto-generated by `rmcs env'. It contains a list of environment\n"
        ";; variables scraped from your default shell (based on your settings for \n"
        ";; `rmcs-env-allow' and `rmcs-env-deny').\n"
        ";;\n"
        (if (file-equal-p env-file rmcs-env-file)
            (concat ";; It is NOT safe to edit this file. Changes will be overwritten next time you\n"
                    ";; run 'rmcs sync'. To create a safe-to-edit envvar file use:\n;;\n"
                    ";;   rmcs env -o ~/.rmcs.d/myenv\n;;\n"
                    ";; And load it with (rmcs-load-envvars-file \"~/.rmcs.d/myenv\").\n")
          (concat ";; This file is safe to edit by hand, but needs to be loaded manually with:\n;;\n"
                  ";;   (rmcs-load-envvars-file \"path/to/this/file\")\n;;\n"
                  ";; Use 'rmcs env -o path/to/this/file' to regenerate it."))
        "\n")
       ...
       (let* ((denylist  (remq nil (append (if deny-only  '(".")) (list allow-only) rmcs-env-deny)))
              (allowlist (remq nil (append (if allow-only '(".")) (list deny-only) rmcs-env-allow))))
         (dolist (rule rules)
         ...
         (dolist (env (get 'process-environment 'initial-value))
           (catch 'skip
             (let* ((var  (car (split-string env "=")))
                    (pred (rmcs-rpartial #'string-match-p var)))
               (when (seq-find pred denylist)
                 (if (seq-find pred allowlist)
                     (rmcs-log "cli:env: allow %s" var)
                   (rmcs-log "cli:env: deny %s" var)
                   (throw 'skip t)))
