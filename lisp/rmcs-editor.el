;;; rmcs-editor.el --- defaults for text editing in Rmcs -*- lexical-binding: t; -*-
;;; Commentary:
;;; Code:

(defvar rmcs-detect-indentation-excluded-modes '(pascal-mode so-long-mode)
  "A list of major modes where indentation shouldn't be auto-detected.")

(defvar-local rmcs-inhibit-indent-detection nil
  "A buffer-local flag that indicates whether `dtrt-indent' should try to detect
indentation settings or not. This should be set by editorconfig if it
successfully sets indent_style/indent_size.")

(defvar rmcs-inhibit-large-file-detection nil
  "If non-nil, inhibit large/long file detection when opening files.")

(defvar rmcs-large-file-p nil)
(put 'rmcs-large-file-p 'permanent-local t)

(defvar rmcs-large-file-size-alist '(("." . 1.0))
  "An alist mapping regexps (like `auto-mode-alist') to filesize thresholds.

If a file is opened and discovered to be larger than the threshold, Rmcs
performs emergency optimizations to prevent Emacs from hanging, crashing or
becoming unusably slow.

These thresholds are in MB, and is used by `rmcs--optimize-for-large-files-a'.")

(defvar rmcs-large-file-excluded-modes
  '(so-long-mode special-mode archive-mode tar-mode jka-compr
    git-commit-mode image-mode doc-view-mode doc-view-mode-maybe
    ebrowse-tree-mode pdf-view-mode tags-table-mode)
  "Major modes that `rmcs-check-large-file-h' will ignore.")


;;
;;; File handling

(defadvice! rmcs--prepare-for-large-files-a (size _ filename &rest _)
  "Sets `rmcs-large-file-p' if the file is considered large.

Uses `rmcs-large-file-size-alist' to determine when a file is too large. When
`rmcs-large-file-p' is set, other plugins can detect this and reduce their
runtime costs (or disable themselves) to ensure the buffer is as fast as
possible."
  :before #'abort-if-file-too-large
  (and (numberp size)
       (null rmcs-inhibit-large-file-detection)
       (ignore-errors
         (> size
            (* 1024 1024
               (assoc-default filename rmcs-large-file-size-alist
                              #'string-match-p))))
       (setq-local rmcs-large-file-p size)))

(add-hook! 'find-file-hook
  (defun rmcs-optimize-for-large-files-h ()
    "Trigger `so-long-minor-mode' if the file is large."
    (when (and rmcs-large-file-p buffer-file-name)
      (if (or rmcs-inhibit-large-file-detection
              (memq major-mode rmcs-large-file-excluded-modes))
          (kill-local-variable 'rmcs-large-file-p)
...
(add-hook! 'find-file-not-found-functions
  (defun rmcs-create-missing-directories-h ()
...
(setq create-lockfiles nil
      ...
      kept-old-versions 5
      kept-new-versions 5
      backup-directory-alist (list (cons "." (concat rmcs-cache-dir "backup/")))
...
(setq auto-save-default t
      ...
      ;; Keep it out of `rmcs-emacs-dir' or the local directory.
      auto-save-list-file-prefix (concat rmcs-cache-dir "autosave/")
      tramp-auto-save-directory  (concat rmcs-cache-dir "tramp-autosave/")
...
(defadvice! rmcs--shut-up-autosave-a (fn &rest args)
...
(defadvice! rmcs-make-hashed-auto-save-file-name-a (fn)
...
(defadvice! rmcs-make-hashed-backup-file-name-a (fn file)
...
(use-package! autorevert
  ...
  :hook (focus-in . rmcs-auto-revert-buffers-h)
  :hook (after-save . rmcs-auto-revert-buffers-h)
  :hook (rmcs-switch-buffer . rmcs-auto-revert-buffer-h)
  :hook (rmcs-switch-window . rmcs-auto-revert-buffer-h)
  :config
  ...
  ;;
  ;; Rmcs does this lazily instead. i.e. All visible buffers are reverted
  ...
  (defun rmcs-auto-revert-buffer-h ()
    "Auto revert current buffer, if necessary."
    (unless (or auto-revert-mode (active-minibuffer-window))
      (let ((auto-revert-mode t))
        (auto-revert-handler))))

  (defun rmcs-auto-revert-buffers-h ()
    "Auto revert stale buffers in visible windows, if necessary."
    (dolist (buf (rmcs-visible-buffers))
      (with-current-buffer buf
        (rmcs-auto-revert-buffer-h)))))


;;;###package bookmark
(setq bookmark-default-file (concat rmcs-data-dir "bookmarks"))


(use-package! recentf
  ;; Keep track of recently opened files
  :defer-incrementally easymenu tree-widget timer
  :hook (rmcs-first-file . recentf-mode)
  :commands recentf-open-files
  :custom (recentf-save-file (concat rmcs-cache-dir "recentf"))
  :config
  (setq recentf-auto-cleanup nil     ; Don't. We'll auto-cleanup on shutdown
        recentf-max-saved-items 200) ; default is 20

  (defun rmcs--recentf-file-truename-fn (file)
    (if (or (not (file-remote-p file))
            (equal "sudo" (file-remote-p file 'method)))
        (abbreviate-file-name (file-truename (tramp-file-name-localname file)))
      file))

  ;; REVIEW: Use this in lieu of `rmcs--recentf-file-truename-fn' when we drop
  ...
  ;; space)
  (add-to-list 'recentf-filename-handlers #'rmcs--recentf-file-truename-fn)

  ;; Text properties inflate the size of recentf's files, and there is
  ;; no purpose in persisting them (Must be first in the list!)
  (add-to-list 'recentf-filename-handlers #'substring-no-properties)

  (add-hook! '(rmcs-switch-window-hook write-file-functions)
    (defun rmcs--recentf-touch-buffer-h ()
  ...
  (add-hook! 'dired-mode-hook
    (defun rmcs--recentf-add-dired-directory-h ()
  ...
  (advice-add #'recentf-load-list :around #'rmcs-shut-up-a))


(use-package! savehist
  ;; persist variables across sessions
  :defer-incrementally custom
  :hook (rmcs-first-input . savehist-mode)
  :custom (savehist-file (concat rmcs-cache-dir "savehist"))
  :config
  ...
  (add-hook! 'savehist-save-hook
    (defun rmcs-savehist-unpropertize-variables-h ()
    ...
    (defun rmcs-savehist-remove-unprintable-registers-h ()
...
(use-package! saveplace
  ...
  :hook (rmcs-first-file . save-place-mode)
  :custom (save-place-file (concat rmcs-cache-dir "saveplace"))
  :config
  (defadvice! rmcs--recenter-on-load-saveplace-a (&rest _)
    "Recenter on cursor when loading a saved place."
    :after-while #'save-place-find-file-hook
    (if buffer-file-name (ignore-errors (recenter))))

  (defadvice! rmcs--inhibit-saveplace-in-long-files-a (fn &rest args)
    :around #'save-place-to-alist
    (unless rmcs-large-file-p
      (apply fn args)))

  (defadvice! rmcs--dont-prettify-saveplace-cache-a (fn)
...
(use-package! server
  ...
  :after-call rmcs-first-input-hook rmcs-first-file-hook focus-out-hook
  :defer 1
  :config
...
(use-package! better-jumper
  :hook (rmcs-first-input . better-jumper-mode)
  :commands rmcs-set-jump-a rmcs-set-jump-maybe-a rmcs-set-jump-h
  :preface
  ...
  :config
  (defun rmcs-set-jump-a (fn &rest args)
  ...
  (defun rmcs-set-jump-maybe-a (fn &rest args)
  ...
  (advice-add #'kill-current-buffer :around #'rmcs-set-jump-a)

  ;; Create a jump point before jumping with imenu.
  (advice-add #'imenu :around #'rmcs-set-jump-a))
...
(use-package! dtrt-indent
  ...
  ;; major mode checks, so I implement it in `rmcs-detect-indentation-h'.
  :hook ((change-major-mode-after-body read-only-mode) . rmcs-detect-indentation-h)
  :config
  (defun rmcs-detect-indentation-h ()
    (unless (or (not after-init-time)
                rmcs-inhibit-indent-detection
                rmcs-large-file-p
                (eq major-mode 'fundamental-mode)
                (member (substring (buffer-name) 0 1) '(" " "*"))
                (apply #'derived-mode-p rmcs-detect-indentation-excluded-modes))
  ...
  (defadvice! rmcs--fix-broken-smie-modes-a (fn &optional arg)
...
(use-package! helpful
  ...
  (defun rmcs-use-helpful-a (fn &rest args)
  ...
  (when (> emacs-major-version 28)
    ...
    (defadvice! rmcs--find-function-search-for-symbol-save-excursion-a (fn &rest args)
...
(use-package! smartparens
  ...
  ;; but that is all Rmcs uses it for.
  :hook (rmcs-first-buffer . smartparens-global-mode)
  :commands sp-pair sp-local-pair sp-with-modes sp-point-in-comment sp-point-in-string
  :config
  (add-to-list 'rmcs-point-in-string-functions 'sp-point-in-string)
  (add-to-list 'rmcs-point-in-comment-functions 'sp-point-in-comment)
  ...
  (add-hook! 'eval-expression-minibuffer-setup-hook
    (defun rmcs-init-smartparens-in-eval-expression-h ()
...
`smartparens-global-mode' is on."
  ...
  (add-hook! 'minibuffer-setup-hook
    (defun rmcs-init-smartparens-in-minibuffer-maybe-h ()
...
on."
  ...
  (defvar rmcs-buffer-smartparens-mode nil)
  (add-hook! 'evil-replace-state-exit-hook
    (defun rmcs-enable-smartparens-mode-maybe-h ()
      (when rmcs-buffer-smartparens-mode
        (turn-on-smartparens-mode)
        (kill-local-variable 'rmcs-buffer-smartparens-mode))))
  (add-hook! 'evil-replace-state-entry-hook
    (defun rmcs-disable-smartparens-mode-maybe-h ()
      (when smartparens-mode
        (setq-local rmcs-buffer-smartparens-mode t)
        (turn-off-smartparens-mode)))))


(use-package! so-long
  :hook (rmcs-first-file . global-so-long-mode)
  :config
  ...
  (if (fboundp 'buffer-line-statistics)
    ...
    (defun rmcs-buffer-has-long-lines-p ()
    ...
    (setq so-long-predicate #'rmcs-buffer-has-long-lines-p))
...
(use-package! ws-butler
  ...
  :hook (rmcs-first-buffer . ws-butler-global-mode)
  :config
