;;; rmcs-projects.el --- defaults for project management in Rmcs -*- lexical-binding: t; -*-
;;; Commentary:
;;; Code:

(defvar rmcs-projectile-cache-limit 10000
  "If any project cache surpasses this many files it is purged when quitting
Emacs.")

(defvar rmcs-projectile-cache-blacklist '("~" "/tmp" "/")
  "Directories that should never be cached.")

(defvar rmcs-projectile-cache-purge-non-projects nil
  "If non-nil, non-projects are purged from the cache on `kill-emacs-hook'.")

(defvar rmcs-projectile-fd-binary
...
(defvar rmcs-projects--fd-version nil)
...
(use-package! projectile
  ...
  :init
  (setq projectile-cache-file (concat rmcs-cache-dir "projectile.cache")
        ...
        projectile-known-projects-file (concat rmcs-cache-dir "projectile.projects")
        projectile-ignored-projects '("~/")
        projectile-ignored-project-function #'rmcs-project-ignored-p)
  ...
  ;;   its `file-name-handler-alist' entry, which is expensive. Since Rmcs
  ...
  (push (abbreviate-file-name rmcs-local-dir) projectile-globally-ignored-directories)
  ...
  (defadvice! rmcs--projectile-dirconfig-file-a ()
  ...
  ;; Disable commands that won't work, as is, and that Rmcs already provides a
  ...
  (add-hook! 'kill-emacs-hook
    (defun rmcs-cleanup-project-cache-h ()
      "Purge projectile cache entries that:

a) have too many files (see `rmcs-projectile-cache-limit'),
b) represent blacklisted directories that are too big, change too often or are
   private. (see `rmcs-projectile-cache-blacklist'),
c) are not valid projectile projects."
      (when (and (bound-and-true-p projectile-projects-cache)
        ...
        (projectile-cleanup-known-projects)
        (cl-loop with blacklist = (mapcar #'file-truename rmcs-projectile-cache-blacklist)
                 for proot in (hash-table-keys projectile-projects-cache)
                 if (or (not (stringp proot))
                        (string-empty-p proot)
                        (>= (length (gethash proot projectile-projects-cache))
                            rmcs-projectile-cache-limit)
                        (member (substring proot 0 -1) blacklist)
                        (and rmcs-projectile-cache-purge-non-projects
                             (not (rmcs-project-p proot)))
                        (projectile-ignored-project-p proot))
                 do (rmcs-log "Removed %S from projectile cache" proot)
  ...
  (defadvice! rmcs--only-use-generic-command-a (fn vcs)
...
And if it's a function, evaluate it."
  ...
  ;; `rmcs--only-use-generic-command-a' advice allows this. I do it this way so
           ...
           ((when-let*
                ...
                (concat (format "%s . -0 -H --color=never --type file --type symlink --follow --exclude .git %s"
                                bin (if (version< version "8.3.0")
                                        "" "--strip-cwd-prefix"))
                        (if rmcs--system-windows-p " --path-separator=/"))))
  ...
  (defadvice! rmcs--projectile-default-generic-command-a (fn &rest args)
    ...
    (macroexp-progn
     (append
      ...
      `((define-minor-mode ,name
          ...
          (if (not ,name)
              ,on-exit
            (run-hook-with-args 'rmcs-project-hook ',name ,name)
            ,(when on-load
               `(unless ,init-var
                  ,on-load
                  (setq ,init-var t)))
      ...
      (cond ((or files modes when)
             ...
             (let ((fn
               ...
               (if modes
                   `((dolist (mode ,modes)
                       (let ((hook-name
                              (intern (format "rmcs--enable-%s%s-h" ',name
...
(provide 'rmcs-projects)
;;; rmcs-projects.el ends here
