...
;;;###autoload
(defun rmcs-session-file (&optional name)
  "TODO"
...
;;;###autoload
(defun rmcs-save-session (&optional file)
  "TODO"
  (setq file (expand-file-name (or file (rmcs-session-file))))
  (cond ((require 'persp-mode nil t)
...
;;;###autoload
(defun rmcs-load-session (&optional file)
  "TODO"
  (setq file (expand-file-name (or file (rmcs-session-file))))
...
;;;###autoload
(defun rmcs/quickload-session (force)
...
;;;###autoload
(defun rmcs/load-session (file)
  "TODO"
  (interactive
   (let ((session-file (rmcs-session-file)))
  ...
  (rmcs-load-session file)
  (message "Session restored. Welcome back."))

;;;###autoload
(defun rmcs/save-session (file)
  "TODO"
  (interactive
   (let ((session-file (rmcs-session-file)))
  ...
  (rmcs-save-session file))

;;;###autoload
(defun rmcs/restart ()
  "Restart Emacs (and the daemon, if active).

Unlike `rmcs/restart-and-restore', does not restart the current session."
  (interactive)
  (require 'restart-emacs)
  (restart-emacs))

;;;###autoload
(defun rmcs/restart-and-restore (&optional debug)
  "Restart Emacs (and the daemon, if active).
...
switch."
  ...
  (letf! ((#'save-buffers-kill-emacs #'kill-emacs)
    ...
    (with-temp-file tmpfile
      (print `(progn
                (when (boundp 'rmcs-version)
                  (add-hook 'window-setup-hook #'rmcs-load-session 100))
                (delete-file ,tmpfile))
             (current-buffer)))
    (restart-emacs
