...
;;;###autoload
(defun rmcs-call-process (command &rest args)
...
;;;###autoload
(defun rmcs-exec-process (command &rest args)
  "Execute COMMAND with ARGS synchronously.

Unlike `rmcs-call-process', this pipes output to `standard-output' on the fly to
    ...
    (cons (let ((process
            ...
            (set-process-filter
             process (lambda (_process output)
                       ...
                       (princ (rmcs-print--format output))))
