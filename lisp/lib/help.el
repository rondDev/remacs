...
(defvar rmcs--help-major-mode-module-alist
  ...
  "An alist mapping major modes to Rmcs modules.

This is used by `rmcs/help-modules' to auto-select the module corresponding to
...
;;;###autoload (defalias 'rmcs/describe-autodefs #'rmcs/help-autodefs)
;;;###autoload (defalias 'rmcs/describe-module   #'rmcs/help-modules)
;;;###autoload (defalias 'rmcs/describe-package  #'rmcs/help-packages)

;;;###autoload
(defun rmcs/describe-active-minor-mode (mode)
  "Get information on an active minor mode. Use `describe-minor-mode' for a
selection of all minor-modes, active or not."
  (interactive
   (list (completing-read "Describe active mode: " (rmcs-active-minor-modes))))
  (let ((symbol
...
(cl-defun rmcs--org-headings (files &key depth mindepth include-files &allow-other-keys)
  "TODO"
  (require 'org)
  (let* ((default-directory rmcs-docs-dir)
...
;;;###autoload
(defun rmcs/homepage ()
  "Open the rmcs emacs homepage in the browser."
  (interactive)
  (browse-url "https://rmcsemacs.org"))

;;;###autoload
(defun rmcs/issue-tracker ()
  "Open Rmcs Emacs' global issue tracker on Discourse."
  (interactive)
  (browse-url "https://git.rmcsemacs.org/todo"))

;;;###autoload
(defun rmcs/report-bug ()
  "Open the browser on our Discourse.

If called when a backtrace buffer is present, it and the output of `rmcs-info'
will be automatically appended to the result."
  (interactive)
  (browse-url "https://git.rmcsemacs.org/core/issues/new?labels=1.+bug%2C2.+status%3Aunread&template=bug_report.yml"))

;;;###autoload
(defun rmcs/discourse ()
  "Open Rmcs Emacs' issue tracker on Discourse."
  (interactive)
  (browse-url "https://discourse.rmcsemacs.org"))

;;;###autoload
(defun rmcs/help ()
  "Open Rmcs's user manual."
  (interactive)
  (find-file (expand-file-name "index.org" rmcs-docs-dir)))

;;;###autoload
(defun rmcs/help-search-headings (&optional initial-input)
  "Search Rmcs's documentation and jump to a headline."
  (interactive)
  (rmcs-completing-read-org-headings
   ...
   :extra-candidates
   (mapcar (lambda (x)
             (setcar x (concat "Rmcs Modules > " (car x)))
             x)
           (rmcs--help-modules-list))))

;;;###autoload
(defun rmcs/help-search (&optional initial-input)
  "Perform a text search on all of Rmcs's documentation."
  (interactive)
...
;;;###autoload
(defun rmcs/help-search-news (&optional initial-input)
  "Search headlines in Rmcs's newsletters."
  (interactive)
  (rmcs-completing-read-org-headings
   "Find in News: "
   (nreverse (rmcs-files-in (expand-file-name "news" rmcs-docs-dir)
                            :match "/[0-9]"
                            :relative-to rmcs-docs-dir))
   :include-files t
   :initial-input initial-input))

;;;###autoload
(defun rmcs/help-faq (&optional initial-input)
  "Search Rmcs's FAQ and jump to a question."
  (interactive)
  (rmcs-completing-read-org-headings
...
;;;###autoload
(defun rmcs/help-news ()
  "Open a Rmcs newsletter.
The latest newsletter will be selected by default."
  (interactive)
  (let* ((default-directory (expand-file-name "news/" rmcs-docs-dir))
         (news-files (rmcs-files-in default-directory)))
    (find-file
     (read-file-name (format "Open Rmcs newsletter (current: v%s): "
                             rmcs-version)
                     default-directory
                     (if (member rmcs-version news-files)
                         rmcs-version
                       (concat (mapconcat #'number-to-string
                                          (nbutlast (version-to-list rmcs-version) 1)
                                          ".")
                               ".x"))
                     t rmcs-version))))

;;;###autoload
(defun rmcs/help-autodefs (autodef)
  "Open documentation for an autodef.

An autodef is a Rmcs concept. It is a function or macro that is always defined,
...
without needing to check if they are available."
            ...
            (cl-loop with maxwidth = (apply #'max (mapcar #'length (mapcar #'symbol-name settings)))
                     ...
                     (format (format "%%-%ds%%s" (+ maxwidth 4))
                             def (propertize (format "%s %s" (car it) (cdr it))
                                  ...
                                  (symbol-file def)
                                  (file-in-directory-p (symbol-file def) rmcs-core-dir))
                     collect
                     (format (format "%%-%ds%%s" (+ maxwidth 4))
                             def (propertize (format "lisp/%s.el" (file-name-sans-extension (file-relative-name (symbol-file def) rmcs-core-dir)))
                                             'face 'font-lock-comment-face)))
            nil t
  ...
  (let ((fn (if (functionp autodef)
              ...
              (intern-soft autodef))))
    (or (fboundp fn)
        (error "'%s' is not a valid RMCS autodef" autodef))
    (if (fboundp 'helpful-callable)
        (helpful-callable fn)
      (describe-function fn))))

(defun rmcs--help-modules-list ()
  (cl-loop for (cat . mod) in (rmcs-module-list 'all)
           for readme-path = (or (rmcs-module-locate-path cat mod "README.org")
                                 (rmcs-module-locate-path cat mod))
           for format = (if mod (format "%s %s" cat mod) (format "%s" cat))
           if (rmcs-module-p cat mod)
           collect (list format readme-path)
           else if (and cat mod)
           collect (list (propertize format 'face 'font-lock-comment-face)
                         readme-path)))

(defun rmcs--help-current-module-str ()
  (cond ((save-excursion
           (require 'smartparens)
           (ignore-errors
        ...
        ((when buffer-file-name
           (when-let (mod (rmcs-module-from-path buffer-file-name))
             (unless (memq (car mod) '(:core :user))
               (format "%s %s" (car mod) (cdr mod))))))
        ((when-let (mod (cdr (assq major-mode rmcs--help-major-mode-module-alist)))
           (format "%s %s"
                   (symbol-name (car mod))
                   (symbol-name (cadr mod)))))))

;;;###autoload
(defun rmcs/help-modules (category module &optional visit-dir)
  "Open the documentation for a Rmcs module.
...
`rmcs--help-major-mode-module-alist')."
  (interactive
   (nconc
    (mapcar #'intern
            (split-string
  ...
  (cl-destructuring-bind (module-string path)
    ...
    (cond ((not (file-directory-p path))
           (if visit-dir
               (rmcs-project-browse (file-name-directory path))
             (find-file path)))
          (visit-dir
           (rmcs-project-browse path))
...
;;;###autoload
(defun rmcs/help-custom-variable (var)
...
(defun rmcs--help-insert-button (label &optional uri line)
...
(defun rmcs--help-package-configs (package)
...
(defvar rmcs--help-packages-list nil)
;;;###autoload
(defun rmcs/help-packages (package)
  "Like `describe-package', but for packages installed by Rmcs modules.
...
If prefix arg is present, refresh the cache."
  (interactive
   (let ((guess (or (function-called-at-point)
     ...
     (require 'straight)
     (let ((packages
            (if (and rmcs--help-packages-list (null current-prefix-arg))
                rmcs--help-packages-list
              (message "Generating packages list for the first time...")
              (redisplay)
              (setq rmcs--help-packages-list
                    (delete-dups
       ...
       (list
        (intern
         (completing-read (format "Describe Rmcs package (%s): "
                                  (concat (when guess
  ...
  (help-setup-xref (list #'rmcs/help-packages package)
  ...
  (with-help-window (help-buffer)
    (with-current-buffer standard-output
      ...
      (let ((indent (make-string 13 ? )))
        ...
        (pcase (rmcs-package-backend package)
          (`straight
           (insert "Straight\n")
           (package--print-help-section "Pinned")
           (insert (if-let (pin (plist-get (cdr (assq package rmcs-packages)) :pin))
                       pin
                     "unpinned")
                   "\n")
           ...
           (let ((default-directory (straight--repos-dir (symbol-name package))))
             (if (file-exists-p default-directory)
                 (insert (cdr (rmcs-call-process "git" "log" "-1" "--format=%D %h %ci")))
               (insert "n/a")))
           (insert "\n" indent)
           ...
           (let ((build-dir (straight--build-dir (symbol-name package))))
             (if (file-exists-p build-dir)
                 (rmcs--help-insert-button (abbreviate-file-name build-dir))
               (insert "n/a")))
           (insert "\n" indent)

           (package--print-help-section "Repo location")
           (let* ((local-repo (rmcs-package-recipe-repo package))
                  (repo-dir (straight--repos-dir local-repo)))
             (if (file-exists-p repo-dir)
                 (rmcs--help-insert-button (abbreviate-file-name repo-dir))
               (insert "n/a"))
             (insert "\n"))

           (let ((recipe (rmcs-package-build-recipe package)))
           ...
           (let ((homepage (rmcs-package-homepage package)))
             (if homepage
                 (rmcs--help-insert-button homepage)
               (insert "n/a"))))

          (`elpa (insert "[M]ELPA ")
                 (rmcs--help-insert-button (rmcs-package-homepage package))
                 (package--print-help-section "Location")
                 (rmcs--help-insert-button
        ...
        (when-let
            (modules
             (if (gethash (symbol-name package) straight--build-cache)
                 (rmcs-package-get package :modules)
               (plist-get (cdr (assq package (rmcs-package-list 'all)))
                          :modules)))
          (package--print-help-section "Modules")
          (insert "Declared by the following Rmcs modules:\n")
          (dolist (m modules)
            (let* ((module-path (pcase (car m)
                                  (:core rmcs-core-dir)
                                  (:user rmcs-user-dir)
                                  (category
                                   (rmcs-module-locate-path category
                                                            (cdr m)))))
              ...
              (if (file-exists-p readme-path)
                  (rmcs--help-insert-button "readme" readme-path)
                (insert "no readme"))
              (insert ")\n"))))

        (package--print-help-section "Configs")
        (if-let ((configs (rmcs--help-package-configs package)))
            (progn
              ...
              (dolist (location configs)
                ...
                (cl-destructuring-bind (file line _match &rest)
                  ...
                  (rmcs--help-insert-button location
                                            (expand-file-name file rmcs-emacs-dir)
                                            (string-to-number line)))))
          (insert "This package is not configured anywhere"))
        (goto-char (point-min))))))

(defvar rmcs--package-cache nil)
(defun rmcs--package-list (&optional prompt)
  (let* ((guess (or (function-called-at-point)
                    (symbol-at-point))))
    (require 'finder-inf nil t)
    (unless package--initialized
      (package-initialize t))
    (let ((packages (or rmcs--package-cache
                        (progn
      ...
      (setq rmcs--package-cache packages)
      (unless (memq guess packages)
        (setq guess nil))
...
;;;###autoload
(defun rmcs/help-package-config (package)
  "Jump to any `use-package!', `after!' or ;;;###package block for PACKAGE.

This only searches `rmcs-emacs-dir' (typically ~/.emacs.d) and does not include
config blocks in your private config."
  (interactive (list (rmcs--package-list "Find package config: ")))
  (cl-destructuring-bind (file line _match)
      (split-string
       (completing-read
        "Jump to config: "
        (or (rmcs--help-package-configs package)
            (user-error "This package isn't configured by you or Rmcs")))
       ":")
    (find-file (expand-file-name file rmcs-emacs-dir))
    (goto-char (point-min))
    (forward-line (1- line))
    (recenter)))

;;;###autoload
(defalias 'rmcs/help-package-homepage #'straight-visit-package-website)

(defun rmcs--help-search-prompt (prompt)
  (let ((query (rmcs-thing-at-point-or-region)))
    (if (featurep 'counsel)
        query
      (read-string prompt query 'git-grep query))))

(defvar counsel-rg-base-command)
(defun rmcs--help-search (dirs query prompt)
...
;;;###autoload
(defun rmcs/help-search-load-path (query)
  "Perform a text search on your `load-path'.
Uses the symbol at point or the current selection, if available."
  (interactive
   (list (rmcs--help-search-prompt "Search load-path: ")))
  (rmcs--help-search (cl-remove-if-not #'file-directory-p load-path)
                     query "Search load-path: "))

;;;###autoload
(defun rmcs/help-search-loaded-files (query)
  "Perform a text search on your `load-path'.
Uses the symbol at point or the current selection, if available."
  (interactive
   (list (rmcs--help-search-prompt "Search loaded files: ")))
  (rmcs--help-search
