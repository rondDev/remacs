...
(defun rmcs-files--build-checks (spec &optional directory)
  "Converts a simple nested series of or/and forms into a series of
`file-exists-p' checks.

For example

  (rmcs-files--build-checks
    '(or A (and B C))
    \"~\")
  ...
  (if (and (listp spec)
      ...
      (cons (car spec)
            (cl-loop for it in (cdr spec)
                     collect (rmcs-files--build-checks it directory)))
...
;;;###autoload
(define-obsolete-function-alias 'rmcs-dir 'rmcs-path "3.0.0")

;;;###autoload
(cl-defun rmcs-files-in
    (paths &rest rest
           &key
           filter
           map
...
MATCH is a string regexp. Only entries that match it will be included."
  (let (result)
    (dolist (file (mapcan (rmcs-rpartial #'rmcs-glob "*") (ensure-list paths)))
...
;;;###autoload
(defun rmcs-file-cookie-p (file &optional cookie null-value)
...
return NULL-VALUE."
  ...
  (with-temp-buffer
    ...
    (if (re-search-forward (format "^;;;###%s " (regexp-quote (or cookie "if")))
                           nil t)
        (rmcs-module-context-with (rmcs-module-from-path file)
...
single file or nested compound statement of `and' and `or' statements."
  `(let ((p ,(rmcs-files--build-checks files directory)))
     (and p (expand-file-name p ,directory))))

;;;###autoload
(defun rmcs-file-size (file &optional dir)
...
;;;###autoload
(defun rmcs-directory-size (dir)
  "Returns the size of FILE (in DIR) in kilobytes."
  (unless (file-directory-p dir)
    (error "Directory %S does not exist" dir))
  (if (executable-find "du")
      (/ (string-to-number (cdr (rmcs-call-process "du" "-sb" dir)))
         1024.0)
        ...
        (unless (member (car attrs) '("." ".."))
          (cl-incf
           sum (if (eq (nth 1 attrs) t) ; is directory
                   (rmcs-directory-size (expand-file-name (car attrs) dir))
                 (/ (nth 8 attrs) 1024.0))))))))
...
(defmacro rmcs--with-prepared-file-buffer (file coding mode &rest body)
    ...
    `(let* ((,nmask ,mode)
       ...
       (unwind-protect
           (with-temp-buffer
             ...
             (let* ((buffer-file-name (rmcs-path ,file))
...
If BEG and/or END are integers, only that region will be read from FILE."
  (when (or (not noerror)
    ...
    (let ((old-buffer (current-buffer)))
      (rmcs--with-prepared-file-buffer file coding nil
...
ends. Set either APPEND or PREPEND to `noerror' to silently ignore read errors."
  (let ((mode (ensure-list mode))
        ...
        datum)
    (rmcs--with-prepared-file-buffer file coding (car mode)
...
optimizations for `binary' IO."
  (declare (indent 1))
  `(rmcs--with-prepared-file-buffer ,file (or coding-system-for-read 'utf-8) nil
     (rmcs-file-read buffer-file-name :by 'insert :coding coding-system-for-read)
     (goto-char (point-min))
     ,@body))
...
some optimizations for `binary' IO."
  ...
  `(rmcs--with-prepared-file-buffer ,file (or coding-system-for-read 'utf-8) nil
     (prog1 (progn ,@body)
       (rmcs-file-write buffer-file-name (current-buffer)
...
(defun rmcs-files--update-refs (&rest files)
    ...
    (dolist (file files)
      ...
      (unless (file-readable-p file)
        ...
        (when (and (bound-and-true-p projectile-mode)
                   (rmcs-project-p)
                   (projectile-file-cached-p file (rmcs-project-root)))
...
;;;###autoload
(defun rmcs/delete-this-file (&optional path force-p)
    ...
    (let ((buf (current-buffer)))
      (unwind-protect
        ...
        (if (file-exists-p path)
          ...
          ;; real buffers (`rmcs-real-buffer-p')
          (rmcs/kill-this-buffer-in-all-windows buf t)
          (rmcs-files--update-refs path)
          (message "Deleted %S" short-path))))))

;;;###autoload
(defun rmcs/copy-this-file (new-path &optional force-p)
...
If FORCE-P, overwrite the destination file if it exists, without confirmation."
  ...
  (let ((old-path (buffer-file-name (buffer-base-buffer)))
    ...
    (find-file new-path)
    (rmcs-files--update-refs old-path new-path)
    (message "File copied to %S" (abbreviate-file-name new-path))))

;;;###autoload
(defun rmcs/move-this-file (new-path &optional force-p)
...
(defun rmcs--sudo-file-path (file)
...
;;;###autoload
(defun rmcs/sudo-find-file (file)
  "Open FILE as root."
  (interactive "FOpen file as root: ")
  (find-file (rmcs--sudo-file-path (expand-file-name file))))

;;;###autoload
(defun rmcs/sudo-this-file ()
...
;;;###autoload
(defun rmcs/sudo-save-buffer ()
  "Save this file as root."
  (interactive)
  (let ((file (rmcs--sudo-file-path buffer-file-name)))
...
;;;###autoload
(defun rmcs/remove-recent-file (file)
