...
;;; Commentary
;;
;; This is Rmcs's output library, for controlling what does and doesn't get
...
;; interactive sessions, but `print!' and `rmcs-log' are safe to use as a
;; drop-in replacement.
;;
;;; Code:
...
`rmcs-print-class-alist').")

(defvar rmcs-print-class-alist
  `((buffer  . rmcs-print--buffer)
    (color   . rmcs-print--style)
    (class   . rmcs-print--class)
    (indent  . rmcs-print--indent)
    (fill    . rmcs-print--fill)
    (join    . rmcs-print--join)
    (org     . rmcs-print--org)
    (markup  . rmcs-print--cli-markup)
    (trim    . string-trim)
    (rtrim   . string-trim-right)
    (ltrim   . string-trim-left)
    (p       . rmcs-print--paragraph)
    (truncate . rmcs-print--truncate)
    (success . (lambda (str &rest args)
                 (apply #'rmcs-print--style 'green
                        (rmcs-print--indent str "✓ ")
                        args)))
    (warn    . (lambda (str &rest args)
                 (apply #'rmcs-print--style 'yellow
                        (rmcs-print--indent str "! ")
                        args)))
    (error   . (lambda (str &rest args)
                 (apply #'rmcs-print--style 'red
                        (rmcs-print--indent str "x ")
                        args)))
...
(defvar rmcs-print-indent 0
  "Level to rigidly indent text returned by `format!' and `print!'.")

(defvar rmcs-print-indent-increment 2
  "Steps in which to increment `rmcs-print-indent' for consecutive levels.")

(defvar rmcs-print-backend (if noninteractive 'ansi 'text-properties)
...
(defvar rmcs-print-level 'notice
  "The current, default logging level.")

(defvar rmcs-print-minimum-level 'notice
...
- Indents according to `rmcs-print-indent' (if FORMAT is non-nil).
...
Returns OUTPUT."
  ...
  (when (and (stringp output)
             (or (eq level t)
                 (if (listp level)
                     (memq rmcs-print-minimum-level level)
                   (>= (get level 'print-level)
                       (get rmcs-print-minimum-level 'print-level)))))
    (when format
      (setq output (rmcs-print--format "%s" output)))
    (princ output stream)
...
(defmacro format! (message &rest args)
  ...
  `(rmcs-print--format ,@(rmcs-print--apply `(,message ,@args))))
...
(defmacro print-group! (&rest body)
  ...
  (cl-destructuring-bind (&key if indent level verbose title
    ...
    `(progn
       ...
       (let ((rmcs-print-level (or ,level rmcs-print-level))
             (rmcs-print-indent
              (+ (if ,(or if t) (or ,indent rmcs-print-indent-increment) 0)
                 rmcs-print-indent)))
         ,@body))))
...
Uses faces in interactive sessions and ANSI codes otherwise."
  `(rmcs-print (format! ,message ,@args)))
...
(defvar rmcs-print--output-depth 0)
;;;###autoload
...
based on the print level of the message. For example:
  ...
  (let ((sym (make-symbol "streamspec")))
    `(letf! ((,sym ,streamspec)
             (standard-output (rmcs-print--redirect-standard-output ,sym t))
             (#'message (rmcs-print--redirect-message ,sym (if noninteractive 'debug 'notice)))
             (rmcs-print--output-depth (1+ rmcs-print--output-depth)))
       ,@body)))
...
(defun rmcs-print--redirect-streams (streamspec level)
...
(defun rmcs-print--redirect-standard-output (streamspec level)
  (let ((old standard-output)
        (streams (rmcs-print--redirect-streams streamspec level)))
    (lambda (ch)
      (let ((str (char-to-string ch)))
        (dolist (stream streams)
          (when (or (eq (car stream) t)
                    (funcall (car stream)))
            (rmcs-print str :newline nil :stream (cdr stream))))
        (rmcs-print str :newline nil :stream t :level level)))))

(defun rmcs-print--redirect-message (streamspec level)
  (let ((old (symbol-function #'message))
        (streams (rmcs-print--redirect-streams streamspec level)))
    (lambda (message &rest args)
      (when message
        (let ((output (apply #'rmcs-print--format message args)))
          (if (<= rmcs-print--output-depth 1)
              (rmcs-print output :level level :stream t)
            (let ((rmcs-print--output-depth (1- rmcs-print--output-depth)))
              (funcall old "%s" output)))
          (dolist (stream streams)
            (when (or (eq (car stream) t)
                      (funcall (car stream)))
              (rmcs-print output :stream (cdr stream)))))
        message))))

;;;###autoload
(defun rmcs-print--format (message &rest args)
  (if (or (null message) (string-blank-p message))
      ""
    (concat (make-string rmcs-print-indent 32)
            (replace-regexp-in-string
             "\n" (concat "\n" (make-string rmcs-print-indent 32))
             (if args (apply #'format message args) message)
             t t))))

;;;###autoload
(defun rmcs-print--indent (text &optional prefix)
...
;;;###autoload
(defun rmcs-print--fill (message &optional column indent)
...
;;;###autoload
(defun rmcs-print--paragraph (&rest lines)
  "TODO"
  (rmcs-print--fill (apply #'concat lines)))

;;;###autoload
(defun rmcs-print--join (sequence &optional separator)
...
;;;###autoload
(defun rmcs-print--truncate (text &optional col ellipsis)
  "Replaces basic org markup with ansi/text-properties."
  (truncate-string-to-width (or text "") (or col (- fill-column rmcs-print-indent))
                            nil nil (or ellipsis "...")))

;;;###autoload
(defun rmcs-print--buffer (buffer &optional beg end)
  "Replaces basic org markup with ansi/text-properties."
...
;;;###autoload
(defun rmcs-print--cli-markup (text)
...
- ```
  ...
  (if (not text) ""
    (let ((case-fold-search nil))
      ...
      (replace-regexp-in-string
       " *```\n\\(.+?\\)\n *```" (rmcs-print--style 'blue "%s" "\\1")
       (replace-regexp-in-string
        "`\\$ \\([^`\n]+?\\)`" (format "`%s`" (rmcs-print--style 'blue "%s" "\\1"))
        (replace-regexp-in-string
         "`\\([^ \n]+?\\)'"
         (let ((styles '(("^\\$" . envvar)
                         ("^--?" . option)
           ...
           (lambda (match)
             (let ((text (match-string 1 match)))
               (pcase (assoc-default text styles #'string-match-p)
                 (`arg    (rmcs-print--style 'underscore "%s" text))
                 (`envvar (rmcs-print--style 'bold "%s" text))
                 (`option (rmcs-print--style 'bold "%s" text))
                 (_ (format "`%s'" (rmcs-print--style 'blue "%s" text)))))))
         text t)
        t)
       t))))

;;;###autoload
(defun rmcs-print--org (text)
  "Replaces basic Org markup with ansi/text-properties.
...
;;;###autoload
(defun rmcs-print--class (class format &rest args)
  "Apply CLASS to formatted format with ARGS.

CLASS is derived from `rmcs-print-class-alist', and can contain any arbitrary,
transformative logic."
  (let (fn)
    (cond ((setq fn (cdr (assq class rmcs-print-class-alist)))
           (if (functionp fn)
...
(defun rmcs-print--apply (forms &optional sub)
  "Replace color-name functions with calls to `rmcs-print--style'."
  (cond ((null forms) nil)
        ((listp forms)
         (append (cond ((not (symbolp (car forms)))
                        (list (rmcs-print--apply (car forms))))
                       (sub
                        (list (car forms)))
                       ((assq (car forms) rmcs-print-ansi-alist)
                        `(rmcs-print--style ',(car forms)))
                       ((assq (car forms) rmcs-print-class-alist)
                        `(rmcs-print--class ',(car forms)))
                       ((list (car forms))))
                 (rmcs-print--apply (cdr forms) t)
                 nil))
        (forms)))

(provide 'rmcs-lib '(print))
;;; print.el ends here
