...
;;;###autoload
(defvar rmcs-point-in-comment-functions ()
...
function to return non-nil. Used by `rmcs-point-in-comment-p'.")

;;;###autoload
(defvar rmcs-point-in-string-functions ()
  "List of functions to run to determine if point is in a string.

Each function takes one argument: the position of the point. Stops on the first
function to return non-nil. Used by `rmcs-point-in-string-p'.")

;;;###autoload
(defun rmcs-surrounded-p (pair &optional inline balanced)
...
;;;###autoload
(defun rmcs-point-in-comment-p (&optional pos)
  "Return non-nil if POS is in a comment.
POS defaults to the current position."
  (let ((pos (or pos (point))))
    (if rmcs-point-in-comment-functions
        (run-hook-with-args-until-success 'rmcs-point-in-comment-functions pos)
      (nth 4 (syntax-ppss pos)))))

;;;###autoload
(defun rmcs-point-in-string-p (&optional pos)
  "Return non-nil if POS is in a string."
  ;; REVIEW Should we cache `syntax-ppss'?
  (let ((pos (or pos (point))))
    (if rmcs-point-in-string-functions
        (run-hook-with-args-until-success 'rmcs-point-in-string-functions pos)
      (nth 3 (syntax-ppss pos)))))

;;;###autoload
(defun rmcs-point-in-string-or-comment-p (&optional pos)
  "Return non-nil if POS is in a string or comment."
  (or (rmcs-point-in-string-p pos)
      (rmcs-point-in-comment-p pos)))

;;;###autoload
(defun rmcs-region-active-p ()
...
;;;###autoload
(defun rmcs-thing-at-point-or-region (&optional thing prompt)
...
(defun rmcs--bol-bot-eot-eol (&optional pos)
    ...
    (let* ((bol (if visual-line-mode
           ...
           (eot (or (save-excursion
                      (if (not comment-use-syntax)
                        ...
                        (goto-char eol)
                        (while (and (rmcs-point-in-comment-p)
                                    (> (point) bol))
                          (backward-char))
...
(defvar rmcs--last-backward-pt nil)
;;;###autoload
(defun rmcs/backward-to-bol-or-indent (&optional point)
  "Jump between the indentation column (first non-whitespace character) and the
beginning of the line. The opposite of
`rmcs/forward-to-last-non-comment-or-eol'."
  (interactive "^d")
  (let ((pt (or point (point))))
    (cl-destructuring-bind (bol bot _eot _eol)
        (rmcs--bol-bot-eot-eol pt)
      (cond ((> pt bot)
             (goto-char bot))
            ((= pt bol)
             (or (and rmcs--last-backward-pt
                      (= (line-number-at-pos rmcs--last-backward-pt)
                         (line-number-at-pos pt)))
                 (setq rmcs--last-backward-pt nil))
             (goto-char (or rmcs--last-backward-pt bot))
             (setq rmcs--last-backward-pt nil))
            ((<= pt bot)
             (setq rmcs--last-backward-pt pt)
             (goto-char bol))))))

(defvar rmcs--last-forward-pt nil)
;;;###autoload
(defun rmcs/forward-to-last-non-comment-or-eol (&optional point)
  "Jumps between the last non-blank, non-comment character in the line and the
true end of the line. The opposite of `rmcs/backward-to-bol-or-indent'."
  (interactive "^d")
  (let ((pt (or point (point))))
    (cl-destructuring-bind (_bol _bot eot eol)
      ...
      (cond ((< pt eot)
             (goto-char eot))
            ((= pt eol)
             (goto-char (or rmcs--last-forward-pt eot))
             (setq rmcs--last-forward-pt nil))
            ((>= pt eot)
             (setq rmcs--last-backward-pt pt)
             (goto-char eol))))))

;;;###autoload
(defun rmcs/backward-kill-to-bol-and-indent ()
...
;;;###autoload
(defun rmcs/delete-backward-word (arg)
...
;;;###autoload
(defun rmcs/retab (arg &optional beg end)
...
;;;###autoload
(defun rmcs/delete-trailing-newlines ()
...
;;;###autoload
(defun rmcs/toggle-indent-style ()
...
;;;###autoload
(defun rmcs/set-indent-width (width)
        ...
        ((require 'dtrt-indent nil t)
         (when-let (vars (nth 2 (assq major-mode dtrt-indent-hook-mapping-list)))
           (dolist (var (ensure-list vars))
             (rmcs-log "Updated %s = %d" var width)
             (set var width)))))
...
;;;###autoload
(defun rmcs-enable-delete-trailing-whitespace-h ()
...
;;;###autoload
(defun rmcs-disable-delete-trailing-whitespace-h ()
...
;;;###autoload
(defun rmcs-enable-show-trailing-whitespace-h ()
  "Enable `show-trailing-whitespace' in the current buffer."
  (setq-local show-trailing-whitespace t))

;;;###autoload
(defun rmcs-disable-show-trailing-whitespace-h ()
