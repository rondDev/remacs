...
;;;###autoload
(defun rmcs-system-distro ()
  "Return a symbol representing the installed distro."
  (with-memoization (get 'rmcs-system-distro 'cached-value)
    (cond (rmcs--system-windows-p 'windows)
          (rmcs--system-macos-p     'macos)
          ((ignore-errors
       ...
       ((ignore-errors
          (with-file-contents! "/etc/os-release"
            (when (re-search-forward "^PRETTY_NAME=\"?\\([^\"\n]+\\)\"?" nil t)
              (match-string 1)))))
       ((when-let (files (rmcs-glob "/etc/*-release"))
          (truncate-string-to-width
           (replace-regexp-in-string
            "\n" " " (rmcs-file-read (car files) :end 73) nil t)
           64 nil nil "...")))
       ((concat "Unknown " (sh "uname" "-v")))))))

;;;###autoload
(defun rmcs-system-distro-icon ()
  "Display icon for the installed distro."
  (with-memoization (get 'rmcs-system-distro-icon 'cached-value)
    (propertize
     (pcase (rmcs-system-distro)
...
Tries to be portable. Returns 1 if cannot be determined."
  ...
  (with-memoization (get 'rmcs-system-cpus 'cached-value)
      ...
      (let ((cpus
             (cond ((fboundp 'w32-get-nproc)
                   ...
                   ((executable-find "nproc")
                    (rmcs-call-process "nproc"))
                   ((executable-find "sysctl")
                    (rmcs-call-process "sysctl" "-n" "hw.ncpu")))))
        (max
