...
- [[#install][Install]]
  ...
  - [[#rmcs-emacs][Doom Emacs]]
    - [[#the-binrmcs-utility][The ~bin/rmcs~ utility]]
    - [[#install-rmcs-manually][Install Doom Manually]]
    - [[#install-rmcs-alongside-other-configs-with-chemacs2][Install Doom alongside other configs (with Chemacs2)]]
...
- [[#configure][Configure]]
  ...
  - [[#configuring-rmcs][Configuring Doom]]
  ...
  - [[#writing-your-own-modules][Writing your own modules]]
    ...
    - [[#flags][Flags]]
    - [[#rmcs-cookies][Doom cookies]]
      - [[#if][~;;;###if~]]
      - [[#package][~;;;###package~]]
      - [[#autodef][~;;;###autodef~]]
  - [[#common-mistakes-when-configuring-rmcs-emacs][Common mistakes when configuring Doom Emacs]]
...
- [[#troubleshoot][Troubleshoot]]
  - [[#looking-up-documentation-and-state-from-within-emacs][Looking up documentation and state from within Emacs]]
    ...
    - [[#for-rmcs-modules-packages-autodefs-etc][For Doom Modules, packages, autodefs, etc.]]
  ...
  - [[#testing-in-rmcss-sandbox][Testing in Doom's sandbox]]
  ...
  - [[#bisecting-rmcs-emacs][Bisecting Doom Emacs]]
...
+ ~rmcs sync~: This synchronizes your config with Doom Emacs. It ensures that
...
+ ~rmcs upgrade~: Updates Doom Emacs (if available) and all its packages.
...
+ ~rmcs doctor~: If Doom misbehaves, the doc will diagnose common issues with
  your installation, system and environment.
+ ~rmcs purge~: Over time, the repositories for Doom's plugins will accumulate.
...
If you'd rather install Doom yourself, instead of rely on the magic of =rmcs
...
# The init.example.el file contains an example rmcs! call, which tells Doom what
...
If you want to update Doom manually, ~rmcs upgrade~ is equivalent to:
#+BEGIN_SRC bash
cd ~/.emacs.d
git pull        # updates Doom
rmcs clean      # Ensure your config isn't byte-compiled
rmcs sync       # synchronizes your config with Doom Emacs
...
=~/.rmcs.d=). Read the [[#Configure][Configure]] section for more on configuring Doom.
#+end_quote

** TODO Rollback
The =bin/rmcs= script doesn't currently offer rollback support for Doom or its
packages (yet).
...
* Configure
You can configure Doom by tweaking the files found in your =RMCSDIR=. Doom
...
#+begin_quote
Change the =RMCSDIR= environment variable to change where Doom looks for this
directory. Symlinks will work as well.
#+end_quote

When you ran ~rmcs install~, it deployed a simple Doom configuration to your
=RMCSDIR=, comprised of these three files:

+ init.el :: Where you'll find your ~rmcs!~ block, which controls what Doom
...
+ Preform expensive or error-prone operations; these files are evaluated
  whenever =bin/rmcs= is used; a fatal error in this file can make Doom
...
Doom's autoload file is generated by scanning these files when you execute ~rmcs
sync~.
...
+ Look up errors/warnings [[file:faq.org::Common Issues][on the FAQ]] and [[https://github.com/hlissner/rmcs-emacs/issues][Doom's issue tracker]]. It is possible
...
+ ~bin/rmcs clean~ will ensure the problem isn't stale bytecode in your private
  config or Doom core. If you haven't used ~bin/rmcs compile~, there's no need
  to do this.
...
+ If possible, see if the issue can be reproduced in vanilla Emacs (Emacs
  without Doom) and/or vanilla Doom (Doom without your private config). [[#testing-in-rmcss-sandbox][Doom's
