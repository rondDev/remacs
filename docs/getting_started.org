...
=rmcs install= will set up your =DOOMDIR= at =~/.rmcs.d= (if it doesn't already
...
easily by tweaking your ~rmcs!~ block (found in =$DOOMDIR/init.el=).

#+begin_quote
If =$DOOMDIR/init.el= doesn't exist, you haven't run ~rmcs install~ yet. See [[#install][the
...
;; your DOOMDIR. e.g. ~/.rmcs.d/lisp/package
