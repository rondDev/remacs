...
GNU Emacs is one grand ol' adventure, let alone Rmcs Emacs. Before you start
you'll need to set up Emacs, Rmcs, and its packages, then learn how to take care
...
contributors who want a deeper understanding of how Rmcs Emacs works.
...
- [[#install][Install]]
  ...
  - [[#rmcs-emacs][Rmcs Emacs]]
    - [[#the-binrmcs-utility][The ~bin/rmcs~ utility]]
    - [[#install-rmcs-manually][Install Rmcs Manually]]
    - [[#install-rmcs-alongside-other-configs-with-chemacs2][Install Rmcs alongside other configs (with Chemacs2)]]
...
- [[#configure][Configure]]
  ...
  - [[#configuring-rmcs][Configuring Rmcs]]
  ...
  - [[#writing-your-own-modules][Writing your own modules]]
    ...
    - [[#flags][Flags]]
    - [[#rmcs-cookies][Rmcs cookies]]
      - [[#if][~;;;###if~]]
      - [[#package][~;;;###package~]]
      - [[#autodef][~;;;###autodef~]]
  - [[#common-mistakes-when-configuring-rmcs-emacs][Common mistakes when configuring Rmcs Emacs]]
...
- [[#troubleshoot][Troubleshoot]]
  - [[#looking-up-documentation-and-state-from-within-emacs][Looking up documentation and state from within Emacs]]
    ...
    - [[#for-rmcs-modules-packages-autodefs-etc][For Rmcs Modules, packages, autodefs, etc.]]
  - [[#how-to-extract-a-backtrace-from-an-error][How to extract a backtrace from an error]]
    ...
    - [[#a-backtrace-from-binrmcs][A backtrace from ~bin/rmcs~]]
  ...
  - [[#testing-in-rmcss-sandbox][Testing in Rmcs's sandbox]]
  ...
  - [[#bisecting-rmcs-emacs][Bisecting Rmcs Emacs]]
...
***** Other dependencies
Then install Rmcs's other dependencies:
#+BEGIN_SRC bash
...
[[https://github.com/hlissner/rmcs-emacs/issues/4876][issue #4876]])
#+begin_src sh
echo "app-editors/emacs gui xft" >> /etc/portage/package.use/emacs
#+end_src

To install the latest unmasked version compatible with Rmcs:
#+begin_src sh
...
suited to Rmcs. Before we get to that you'll need either the Homebrew or
...
to least recommended for Rmcs (based on compatibility).
...
These builds/forks have known compatibility issues with Rmcs and are *very
...
necessary for Rmcs's installation process. You'll have to do so yourself by
...
2. Add =C:\Users\USERNAME\.emacs.d\bin= to your ~PATH~.
   ...
   This way, you don't have to type all of =C:\Users\USERNAME\.emacs.d\bin\rmcs=
   ...
   ~pathA:pathB:pathC~. Prepend the path to bin/rmcs to that string, like so:
...
**** With a precompiled binary + Git Bash
(Credit goes to @earvingad and [[https://earvingad.github.io/posts/rmcs_emacs_windows/][his fantastic tutorial]] for informing this guide)
...
(Credit goes to @lunias and [[https://ethanaa.com/blog/switching-to-rmcs-emacs/#installing-on-windows-10
...
** Rmcs Emacs
With Emacs and Rmcs's dependencies installed, next is to install Rmcs Emacs
itself:

#+BEGIN_SRC bash
git clone https://github.com/hlissner/rmcs-emacs ~/.emacs.d
~/.emacs.d/bin/rmcs install
#+END_SRC

=rmcs install= will set up your =RMCSDIR= at =~/.rmcs.d= (if it doesn't already
exist) and will work you through the first-time setup of Rmcs Emacs. Carefully
follow any instructions it puts out.

If this is your first time, you should run ~rmcs doctor~. This will diagnose
common issues with your system or config.

#+BEGIN_QUOTE
If you'd like a more technical break down of ~rmcs install~, it's been
translated into shell commands below, in the "Install Rmcs Manually" section.
#+END_QUOTE

*** The ~bin/rmcs~ utility
This utility is your new best friend. It won't spot you a beer, but it'll
shoulder much of the work associated with managing and maintaining your Rmcs
Emacs configuration, and then some. Not least of which is installation of and
updating Rmcs and your installed packages.

It exposes a variety of commands. ~bin/rmcs help~ will list them all, but here
is a summary of the most important ones:

+ ~rmcs sync~: This synchronizes your config with Rmcs Emacs. It ensures that
  needed packages are installed, orphaned packages are removed and necessary
  metadata correctly generated. Run this whenever you modify your ~rmcs!~ block
  or =packages.el= file. You'll need ~rmcs sync -u~ if you override the recipe
  of package installed by another module.
+ ~rmcs upgrade~: Updates Rmcs Emacs (if available) and all its packages.
+ ~rmcs env~: (Re)generates an "envvar file", which is a snapshot of your
  shell environment that Rmcs loads at startup. If your app launcher or OS
  launches Emacs in the wrong environment you will need this. **This is required
  for GUI Emacs users on MacOS.**
+ ~rmcs doctor~: If Rmcs misbehaves, the doc will diagnose common issues with
  your installation, system and environment.
+ ~rmcs purge~: Over time, the repositories for Rmcs's plugins will accumulate.
...
Use ~rmcs help~ to see an overview of the available commands that =rmcs=
provides, and ~rmcs help COMMAND~ to display documentation for a particular
~COMMAND~.

#+begin_quote
I recommend you add =~/.emacs.d/bin= to your ~PATH~ so you can call =rmcs=
directly and from anywhere. Accomplish this by adding this to your .bashrc or
.zshrc file: ~export PATH="$HOME/.emacs.d/bin:$PATH"~
#+end_quote

*** Install Rmcs Manually
If you'd rather install Rmcs yourself, instead of rely on the magic of =rmcs
install=, here is its equivalent in bash shell commands (assuming
=hlissner/rmcs-emacs= has been cloned to =~/.emacs.d=):

#+BEGIN_SRC bash
# So we don't have to write ~/.emacs.d/bin/rmcs every time
PATH="$HOME/.emacs.d/bin:$PATH"

# Create a directory for our private config
mkdir ~/.rmcs.d  # or ~/.config/rmcs

# The init.example.el file contains an example rmcs! call, which tells Rmcs what
# modules to load and in what order.
cp ~/.emacs.d/init.example.el ~/.rmcs.d/init.el
cp ~/.emacs.d/core/templates/config.example.el ~/.rmcs.d/config.el
cp ~/.emacs.d/core/templates/packages.example.el ~/.rmcs.d/packages.el

# You might want to edit ~/.rmcs.d/init.el here and make sure you only have the
# modules you want enabled.

# Then synchronize Rmcs with your config:
rmcs sync
...
# shell) then create an envvar file to ensure Rmcs correctly inherits your shell
# environment.
#
# If you don't know whether you need this or not, there's no harm in doing it
# anyway. `rmcs install` will have prompted you to generate one. If you
# responded no, you can generate it later with the following command:
rmcs env

# Lastly, install the icon fonts Rmcs uses:
emacs --batch -f nerd-icons-install-fonts
#+END_SRC

To understand the purpose of the =~/.rmcs.d= directory and =~/.rmcs.d/init.el=
file, see the [[#configure][Configure]] section further below.

*** Install Rmcs alongside other configs (with Chemacs2)
[[https://github.com/plexus/chemacs2][Chemacs2]] is a bootloader for Emacs. It allows you to switch between multiple
Emacs configurations. Here is a quick guide for setting it up with Rmcs Emacs as
the default config:

1. First, install Rmcs somewhere:
   #+BEGIN_SRC sh :eval no
   git clone https://github.com/hlissner/rmcs-emacs ~/rmcs-emacs
   ~/rmcs-emacs/bin/rmcs install
   #+END_SRC
...
3. Create =~/.emacs-profiles.el= with a list of your Emacs profiles. This file
   is structured like a =.dir-locals.el= file. Here is an example with Rmcs (as
   the default), Spacemacs, and Prelude:
   #+BEGIN_SRC emacs-lisp :eval no
   (("default"   . ((user-emacs-directory . "~/rmcs-emacs")))
...
Rmcs is comprised of approximately 160 modules which provide its features,
...
The [[file:modules.org][Module Index]] lists all Rmcs's available modules, with links to their
documentation. Documentation is a work-in-progrees; some modules may not have
README.org files yet!

#+begin_quote
Use ~M-x rmcs/help-modules~ (bound to =SPC h d m= or =C-h d m=) to jump to a
module's documentation from within Rmcs, otherwise, place your cursor on a
module in your ~rmcs!~ block (in =~/.rmcs.d/init.el=) and press =K= to jump to
its documentation (or =gd= to jump to its source code). =C-c g k= and =C-c g d=
for non-evil users, respectively.
#+end_quote

* Update & Rollback
Rmcs is an active project and many of its 300+ packages are in active
development as well. It is wise to occasionally update:
#+BEGIN_SRC bash
rmcs upgrade   # or 'rmcs up'
#+END_SRC

If you want to update Rmcs manually, ~rmcs upgrade~ is equivalent to:
#+BEGIN_SRC bash
cd ~/.emacs.d
git pull        # updates Rmcs
rmcs clean      # Ensure your config isn't byte-compiled
rmcs sync       # synchronizes your config with Rmcs Emacs
rmcs update     # updates installed packages
#+END_SRC

To upgrade only your packages (and not Rmcs itself):

#+BEGIN_SRC bash
rmcs upgrade --packages
#+END_SRC

#+begin_quote
To minimize issues while upgrading, avoid modifying Rmcs's source files in
=~/.emacs.d=. All your customization should be kept in your =RMCSDIR= (e.g.
=~/.rmcs.d=). Read the [[#Configure][Configure]] section for more on configuring Rmcs.
#+end_quote

** TODO Rollback
The =bin/rmcs= script doesn't currently offer rollback support for Rmcs or its
packages (yet).

** Up/Downgrading Emacs
*You may encounter errors after up/downgrading Emacs.* Run ~rmcs sync~ on the
command line after changing the installed version of Emacs. If you've changed
the major version (e.g. 27 -> 28 or vice versa) run ~rmcs build~ too.

+ ~rmcs sync~ will re-index any built-in/site loaddef files. This is especially
  necessary if paths to built-in libraries have changed.
+ ~rmcs build~ will recompile all your installed packages, which is necessary
  ...
  =~/.emacs.d/.local=, then run ~rmcs sync~.

* TODO Migrate
If you're here from another Emacs distribution (or your own), here are a few
things to be aware of while you convert your old config to Rmcs:

+ Rmcs does not use =package.el= to manage its packages, but ~use-package~ does!
  ...
  =~/.rmcs.d/packages.el= to install your packages.
...
* Configure
You can configure Rmcs by tweaking the files found in your =RMCSDIR=. Rmcs
expects this directory to be found in one of:

1. =~/.config/rmcs= (respects ~$XDG_CONFIG_HOME~)
2. or =~/.rmcs.d=

This directory is referred to as your =RMCSDIR=. Only one of these directories
should exist (Rmcs will only recognize one).

#+begin_quote
Change the =RMCSDIR= environment variable to change where Rmcs looks for this
directory. Symlinks will work as well.
#+end_quote

When you ran ~rmcs install~, it deployed a simple Rmcs configuration to your
=RMCSDIR=, comprised of these three files:

+ init.el :: Where you'll find your ~rmcs!~ block, which controls what Rmcs
  ...
  targeting Rmcs's CLI or something that needs to be configured very early in
  the startup process.
...
#+begin_quote
Note: do not use ~M-x customize~ or the customize API in general. Rmcs is
...
If you're concerned about ~defcustom~ setters, Rmcs has a ~setq!~ macro that
will trigger them.
#+end_quote

** Modules
Rmcs consists of around 160 modules and growing. A Rmcs module is a bundle of
packages, configuration and commands, organized into a unit that can be toggled
easily by tweaking your ~rmcs!~ block (found in =$RMCSDIR/init.el=).

#+begin_quote
If =$RMCSDIR/init.el= doesn't exist, you haven't run ~rmcs install~ yet. See [[#install][the
"Install" section]] above.
#+end_quote

Your ~rmcs!~ block should look something like this:
...
#+begin_quote
*IMPORTANT:* any changes to your ~rmcs!~ block won't take effect until you run
 ~rmcs sync~ on the command line.
#+end_quote

#+begin_quote
~rmcs doctor~ will detect issues with your ~rmcs!~ block, such as duplicate or
misspelled modules and flags.
#+end_quote

** Package management
**Rmcs Emacs does not use package.el** (the package manager built into Emacs).
Instead, it uses its own declarative package manager built on top of
[[https://github.com/raxod502/straight.el][straight.el]].

Packages are declared in ~packages.el~ files. You'll find one in your =RMCSDIR=
and in many of Rmcs's modules. Read on to learn how to use this system to
...
time you run ~rmcs sync~ or ~rmcs purge~.
#+end_quote
...
#+begin_quote
*IMPORTANT:* New packages won't be installed until you run ~rmcs sync~.
#+end_quote
...
#+begin_quote
*IMPORTANT:* Run ~bin/rmcs sync~ whenever you modify packages.el files to
ensure your changes take effect.
#+end_quote

*** Pinning packages to specific commits
All of Rmcs's packages are pinned by default. A pinned package is a package
...
Though it is *highly* discouraged, you may unpin all packages and make Rmcs
...
#+begin_quote
Unpinning all packages is discouraged because Rmcs's modules are designed
...
ignored, and the package is removed the next time you run ~bin/rmcs sync~. Use
this to disable Rmcs's packages that you don't want or need.
...
If a Rmcs module installs package X from one place, but you'd like to install it
from another (say, a superior fork), add a ~package!~ declaration for it in your
=RMCSDIR/packages.el=. Your private declarations always have precedence over
...
#+begin_quote
*IMPORTANT:* Remember to run ~rmcs sync -u~ after changing recipes for existing
packages. At the time of writing, ~rmcs sync~ alone will not pick up on recipe
changes.
#+end_quote
...
;; or ~/.rmcs.d/{local-repo} -- the first that is found.
(package! my-package
  :recipe (:local-repo "my/package"))  ; looks for ~/.rmcs.d/my/package/my-package.el
...
(package! my-package
  :recipe (:local-repo "/path/to/my/package"
           ...
           ;; With this you can avoid having to run 'rmcs sync' every time you
           ;; change the package.
           :build (:not compile)))
#+END_SRC
...
#+BEGIN_SRC elisp
;; Rmcs has modified `use-package's `:load-path' to expand relative paths from
;; your RMCSDIR. e.g. ~/.rmcs.d/lisp/package
...
#+begin_quote
*IMPORTANT:* Remember to run ~rmcs sync~ to rebuild your package after you've
...
to disable, append/prepend to and/or overwrite Rmcs's ~use-package!~ blocks.
...
#+BEGIN_SRC emacs-lisp
;;; ~/.rmcs.d/init.el (example)
;; If a :pre-init / :pre-config hook returns nil, it overwrites that package's
;; original :init / :config block. Exploit this to overwrite Rmcs's config.
(use-package-hook! rmcs-themes
  :pre-config
  (setq rmcs-neotree-file-icons t)
  nil)
...
that don't require a complete restart of Rmcs Emacs (like changing your enabled
...
=~/.rmcs.d/modules/abc/xyz=, then add =:abc xyz= to your ~rmcs!~ block in
=~/.rmcs.d/init.el= to enable it.

#+begin_quote
In this example, =:abc= is called the category and =xyz= is the name of the
module. Rmcs refers to modules in one of two formats: =:abc xyz= and =abc/xyz=.
#+end_quote

If a private module possesses the same name as a built-in Rmcs module (say,
...
The remainder of this guide will go over the technical details of a Rmcs module.

*** File structure
Rmcs recognizes a handful of special file names, none of which are required for
...
**** =init.el=
This file is loaded early, before anything else, but after Rmcs core is loaded.
It is loaded in both interactive and non-interactive sessions (it's the only
file, besides =cli.el= that is loaded when the =bin/rmcs= starts up).
...
+ Reconfigure packages defined in Rmcs modules with ~use-package-hook!~ (as a
  last resort, when ~after!~ and hooks aren't enough).
+ Configure behavior of =bin/rmcs= in a way that must also apply in
...
+ Preform expensive or error-prone operations; these files are evaluated
  whenever =bin/rmcs= is used; a fatal error in this file can make Rmcs
  unbootable (but not irreversibly).
+ Define new =bin/rmcs= commands here. That's what =cli.el= is for.
...
+ Use ~after!~ or ~use-package!~ to configure packages.
  ...
  (use-package! company  ; `use-package!' is a thin wrapper around `use-package'
                         ; it is required that you use this in Rmcs's modules,
...
Don't:
+ Use ~package!~
+ Install packages with =package.el= or ~use-package~'s ~:ensure~ property. Rmcs
...
1. Add a ~autoload~ call to Rmcs's autoload file (found in
   =~/.emacs.d/.local/autoloads.el=, which is read very early in the startup
   process).
2. Or copy that lisp form to Rmcs's autoload file verbatim (usually the case for
   anything other than ~def*~ forms, like ~defun~ or ~defmacro~).

Rmcs's autoload file is generated by scanning these files when you execute ~rmcs
sync~.
...
**** =doctor.el=
When you execute ~rmcs doctor~, this file defines a series of tests for the
...
**** TODO =cli.el=
This file is read when =bin/rmcs= starts up. Use it to define your own CLI
commands or reconfigure existing ones.

**** TODO =test/**/test-*.el=
Rmcs's unit tests go here. More information on them to come...
...
in Rmcs's community that you may choose to adopt: extra files in the root of the
...
what kind of session it is. Rmcs has three types of sessions:
...
+ CLI session :: this is the same as a batch session /except/ it is what starts
  up when you run any =bin/rmcs= command.

With that out of the way, here is the load order of Rmcs's most important files:

| File                                        | Interactive | Batch | CLI |
|---------------------------------------------+-------------+-------+-----|
| ~/.emacs.d/early-init.el (Emacs 27+ only)   | yes         | no    | no  |
| ~/.emacs.d/init.el                          | yes         | no    | no  |
| $RMCSDIR/init.el                            | yes         | yes   | yes |
| {~/.emacs.d,$RMCSDIR}/modules/*/*/init.el   | yes         | yes   | yes |
| $RMCSDIR/cli.el                             | no          | no    | yes |
| {~/.emacs.d,$RMCSDIR}/modules/*/*/cli.el    | no          | no    | yes |
| {~/.emacs.d,$RMCSDIR}/modules/*/*/config.el | yes         | no    | no  |
| $RMCSDIR/config.el                          | yes         | no    | no  |
...
to Rmcs Emacs. This section will go over what they do and how to use them.
...
determine whether or not to include this file for autoloads scanning (on ~rmcs
sync~) or byte-compilation (on ~rmcs compile~).

i.e. if =FORM= returns ~nil~, Rmcs will neither index its ~;;;###autoload~
...
**** ~;;;###package~
This cookie exists solely to assist the ~rmcs/help-packages~ command. This
...
Use it to let ~rmcs/help-packages~ know where to find config for packages where
no ~after!~ or ~use-package!~ call is involved.

**** ~;;;###autodef~
An autodef is a special kind of autoloaded function (or macro) which Rmcs
...
rmcs/help-autodefs~ (=SPC h d u= or =C-h d u=).
...
** Common mistakes when configuring Rmcs Emacs
Having helped many users configure Rmcs, I've spotted a few recurring oversights
...
many of Rmcs's startup optimizations.
...
keyword). You are free to do this, if it is your preference, but otherwise, Rmcs
has its own package management system.

Migrating ~use-package~ code to Rmcs is usually a case of removing the ~:ensure~
keyword and adding a ~(package! PACKAGENAME)~ to =~/.rmcs.d/packages.el= (and
running ~rmcs sync~ to sync your config).

*** Using ~org-babel-do-load-languages~ to load your babel packages
You don't need ~org-babel-do-load-languages~. Rmcs lazy loads babel packages
...
There may be some special cases, however. Rmcs tries to handle a couple of them
...
for Rmcs Emacs. We already use the more sophisticated =ws-butler= to manage
...
solve them, or for the bug report you're about to write. Both Emacs and Rmcs
...
+ Look up errors/warnings [[file:faq.org::Common Issues][on the FAQ]] and [[https://github.com/hlissner/rmcs-emacs/issues][Rmcs's issue tracker]]. It is possible
  that a solution for your issue already exists. The FAQ can be searched from
  inside Rmcs with =SPC h d f= (or =C-h d f= for non-evil users).

+ Run ~bin/rmcs doctor~ on the command line to diagnose common issues with your
  environment and config. It will suggest solutions for them as well.

+ ~bin/rmcs clean~ will ensure the problem isn't stale bytecode in your private
  config or Rmcs core. If you haven't used ~bin/rmcs compile~, there's no need
  to do this.

+ ~bin/rmcs sync~ will ensure the problem isn't missing packages or outdated
  autoloads files

+ ~bin/rmcs build~ will ensure the problem isn't stale package bytecode or
  broken symlinks.

+ ~bin/rmcs update~ will ensure that your packages are up-to-date, eliminating
...
+ If possible, see if the issue can be reproduced in vanilla Emacs (Emacs
  without Rmcs) and/or vanilla Rmcs (Rmcs without your private config). [[#testing-in-rmcss-sandbox][Rmcs's
  sandbox can help you check]].

+ Ask for help on [[https://discord.gg/qvGgnVx][our Discord server]]. It is the quickest way to get help,
  sometimes straight from Rmcs's maintainer, who is very active there.
...
*** TODO For Rmcs Modules, packages, autodefs, etc.
+ ~rmcs/open-news~ (=SPC h n=) ::
     ...
+ ~rmcs/help~ (=SPC h d h=) ::
     Open the index of Rmcs's manual.
+ ~rmcs/help-modules~ (=SPC h d m=) ::
     Jumps to a module's documentation.
+ ~rmcs/help-autodefs~ (=SPC h u=) ::
...
If you encounter an error while using Rmcs Emacs, you're probably about to head
...
*** A backtrace from ~bin/rmcs~
If the error you've encountered is emitted from ~bin/rmcs~, you can re-run the
...
#+BEGIN_QUOTE
Note: switch order is important. ~-d~ / ~--debug~ /must/ come right after ~rmcs~
...
** Testing in Rmcs's sandbox
"The sandbox" is one of Rmcs Emacs' features; it is a test bed for running elisp
in a fresh instance of Emacs with varying amounts of Rmcs loaded (none at all,
...
Otherwise, it is best to bring it up on the Rmcs Emacs issue list, rather than
confusing and inundating the Emacs community with Rmcs-specific issues.
...
Doing any of the above will pop up a ~*rmcs:sandbox*~ window. What you enter
...
- =C-c C-d= :: This launches "vanilla Rmcs", which is vanilla Emacs plus Rmcs's
  core. This does not load your private config, nor any of Rmcs's (or your)
  modules.
- =C-c C-p= :: This launches "vanilla Rmcs+". That is, Rmcs core plus the
  modules that you have specified in the ~rmcs!~ block of your private config
  (in =~/.rmcs.d/init.el=). This *does not* load your private config, however.
- =C-c C-f= :: This launches "full Rmcs". It loads Rmcs's core, your enabled
