...
:END:
#+title:    Examples
#+subtitle: Samples of Emacs/Rmcs dotfiles, concepts, and sub-projects
...
examples of Rmcs's (and Emacs') concepts, libraries, dotfiles, and more, for
your own reference. They are divided into Emacs-specific and Rmcs-specific
examples; where the former can also be useful to users who don't use Rmcs.

Some of Rmcs's components will read this file to generate documentation for you,
for example:

- Rmcs's [[rmcs-module:][:lang emacs-lisp]] module installs the [[rmcs-package:elisp-demos]] package. This displays
  usage examples alongside documentation in [[rmcs-package:help]] and [[rmcs-package:helpful]] buffers (produced
  by =describe-*= and =helpful-*= commands; e.g. [[kbd:][<help> h f]]). Rmcs has extended
  this package to search this file as well.
- [[id:1b8b8fa9-6233-4ed8-95c7-f46f8e4e2592][Some Rmcs's CLI commands]] will emit documentation informed by Rmcs's org files,
  including this file.
...
all Emacs users, whether or not they use Rmcs.
...
Rmcs and its users. These are intended to be demonstrations, not substitutes for
documentation.
...
((profile1
  ...
  ;; If this is a Rmcs config, you'll also want to set `rmcs-user-dir', which
  ...
  ;; Rmcs profiles support a number of special directives. They are:
  ;;
...
**** Unix utilities, rewritten as Rmcs scripts
To show off the syntax and capabilities of Rmcs's CLI framework, here are some
...
AUTHOR:
  Original program by David MacKenzie. Rmcsscript port by Henrik Lissner.
...
****** Notes
- Docstrings for Rmcs CLIs recognize indented sections with a capitalized
...
- The options were documented in the CLI's docstring, instead of inline like so:
  ...
  docstring. If both were present, Rmcs's help docs would have concatenated them
...
It serves as an example of the bare minimum you need to write a Rmcs-based CLI.
Naturally, it could be more useful; it could process more complex options and
arguments, call other Rmcs CLIs, read/write data from files or over networks --
...
Rmcs's CLI, by reimplementing a subset of ~emacs~'s options and arguments (and
...
(defcli! demacs
  ...
  "Demacs is a thin wrapper around Emacs, made to demo of Rmcs's CLI Framework.
