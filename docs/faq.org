...
#+begin_quote
 󰐃 Our documentation was designed to be read in Rmcs Emacs ([[kbd:][M-x rmcs/help]]) or
...
:END:
This section is for general questions about the Rmcs Emacs project and its
author.

** Is Emacs (or Rmcs) for me?
:PROPERTIES:
...
- [[id:46d2058d-249e-45a7-a7a2-58bade48df65][Why Rmcs?]]
- [[id:da317a3c-1672-40e2-85d5-b4246482af2c][Why /not/ Rmcs?]]

** How does Rmcs compare to <insert starter kit>?
I've compiled comparisons to several starter kits in [[id:f2186c7b-9f48-4b0b-ac0b-d891b5b1e1b3][our migration guide]].

** Is Rmcs a fork of another starter kit?
No. I started Rmcs from scratch in 2013 to learn Emacs and Emacs Lisp, and to
write a private config for myself. Starter kits were brought to my attention
later, when early Rmcs users vocally compared them.

That's not to say I've taken no inspiration from them since. Rmcs's earliest
...
** Rmcs claims to be an Emacs framework, not a starter kit. What's the difference?
I don't claim to own these terms, but I use them to distinguish [[https://git.rmcsemacs.org/core][Rmcs's core]] (a
framework) from [[https://git.rmcsemacs.org/modules][Rmcs's module library]] (a starter kit).
...
- Proper technical documentation for users, contributors, and
  ...
  Rmcs's goals aren't formally stated anywhere.
...
- Resolved its stability, performance, and standardization issues that plague
  ...
  co-maintainers in good conscience. I could live with them when Rmcs was my
...
** Why is the project called "Rmcs"?
As a kid in the Cretaceous period (1999), the [[https://github.com/id-Software/RMCS][source code]] of idsoftware's
classic Rmcs was my first exposure to programming. Totally clueless about C and
...
So "Rmcs Emacs" was what you got when you combined childhood, demon-carnage
...
** Why does Rmcs use straight.el and not package.el?
...
- No disaster recovery ::
  ...
  Though Rmcs doesn't have a 'rmcs rollback' command (yet), the ability to
  quickly checkout an old commit of a problematic package (or Rmcs itself),
...
- Slow at startup (by default) ::
  ...
  ~package-quickstart~ helps, up to a point, but Rmcs's package manager can do
...
Rmcs fills these gaps with =straight.el='s help, and beyond.
...
first thing Rmcs's users notice. Rmcs is perfectly compatible with the daemon
...
That said, there's no one to blame for that. I consider Rmcs's effort, the
...
platforms. I'd recommend [[https://discourse.rmcsemacs.org/get-help][our Discourse]].

** I don't like Discord. Can Rmcs move to IRC/Telegram/Matrix/other?
This is requested *a lot*--and not always so kindly. We have [[https://discourse.rmcsemacs.org][a Discourse]] and
[[https://discourse.rmcsemacs.org/t/a-matrix-space-for-rmcs-emacs/2664/12][there are plans for a Matrix space]], but that's as many platforms as I have
...
:END:
For common how-to and configuration questions from Rmcs users. You'll find more
[[https://discourse.rmcsemacs.org/tags/faq][on our Discourse]].

** Find out what version of Rmcs am I running?
Use ~M-x rmcs/info~ or ~$ rmcs info~ to produce detailed information about your
installation and environment.

Alternatively, ~M-x rmcs/version~ or ~$ rmcs version~ will only list the
versions of installed Rmcs components.

The installed version of Rmcs core is also displayed in the Rmcs dashboard's
mode-line.

** Turn (Rmcs) Emacs into a <insert language here> IDE with LSP?
:PROPERTIES:
:ID:       4cec77b2-bb1c-4b35-8ad1-d6dd9fcc1dbb
:END:
This is frequently asked: how to transform Rmcs Emacs into an IDE for some
programming language. In almost all cases, it's recommended that you enable
Rmcs's LSP support and install a supported LSP client.

This provides code completion, lookup {definition,references,documentation}
functionality, and syntax checking, all in one:

1. Enable Rmcs's [[rmcs-module::tools lsp]] module. ([[id:01cffea4-3329-45e2-a892-95a384ab2338][How do I enable modules?]])
2. Enable the ~+lsp~ flag on the ~:lang~ module corresponding to the language
   you want LSP support for. E.g. For python, enable [[rmcs-module::lang python +lsp]]. For
   rust, enable [[rmcs-module::lang rust +lsp]]. ([[id:01cffea4-3329-45e2-a892-95a384ab2338][How do I enable module flags?]])
...
4. Run ~$ rmcs sync~ on the command line.
5. Restart Emacs.
6. (Optional) If Emacs fails to find your LSP server, you may need to run ~$
   rmcs env~ to regenerate your envvar file (which contains your =$PATH=, which
...
1. Some language modules lack LSP support (either because it doesn't exist or
   ...
   supported by [[rmcs-package:lsp-mode]], simply adding [[fn:lsp!]] to that major mode's hook will be
   enough to enable it:
   ...
   ;; For Elisp gurus: Rmcs provides MAJOR-MODE-local-vars-hook, and we use it
...
documentation with [[kbd:][<help> d m]] (or ~M-x rmcs/help-modules~).

** Change my fonts
Rmcs exposes a couple variables for setting fonts. They are:

- [[var:rmcs-font]]: the primary font for Emacs to use.
- [[var:rmcs-variable-pitch-font]]: used for non-monospace fonts (e.g. when using
  variable-pitch-mode or mixed-pitch-mode). Popular for text modes, like Org or
  Markdown.
- [[var:rmcs-emoji-font]]: used for rendering emoji. Only needed if you want to use
  a font other than your operating system’s default.
- [[var:rmcs-symbol-font]]: used for rendering symbols.
- [[var:rmcs-serif-font]]: the sans-serif font to use wherever the [[face:fixed-pitch-serif]]
  face is used.
- [[var:rmcs-big-font]]: the large font to use when [[fn:rmcs-big-font-mode]] is active.
...
(setq rmcs-font (font-spec :family "JetBrainsMono" :size 12 :weight 'light)
      rmcs-variable-pitch-font (font-spec :family "DejaVu Sans" :size 13)
      rmcs-symbol-font (font-spec :family "JuliaMono")
      rmcs-big-font (font-spec :family "JetBrainsMono" :size 24))
#+end_src
...
3. Then reload the fonts: ~M-x rmcs/reload-font~.

Your changes should take effect immediately.

** Change, customize, or make themes?
- To change themes, add ~(setq rmcs-theme 'name-of-theme)~ to
  =$RMCSDIR/config.el=.
- To switch themes on-the-fly, type [[kbd:][<help> t]] or ~M-x load-theme~.
- To customize or write themes, see our guide on [[https://discourse.rmcsemacs.org/t/how-to-switch-customize-or-write-themes/37][the Discourse]].

** Properly update Rmcs?
You can update Rmcs one of two ways:

1. The correct way: by running ~$ rmcs upgrade~ in the shell and restarting
   Emacs.
2. The manual way (if ~rmcs upgrade~ is broken for some reason):

   #+begin_src bash
   $ cd ~/.emacs.d
   $ git pull       # pull the latest version of Rmcs's source
   $ rmcs sync -u   # update Rmcs's packages and 'rmcs sync'
   #+end_src

This may change in the future, so ~$ rmcs upgrade~ will always be the safest
option. That said, ~$ rmcs help upgrade~ will always document the correct
...
However, Rmcs provides a more general ~map!~ macro, to conveniently wrap up the
...
detailed guide on keys, keymaps, and keymap precedence can be found [[https://discourse.rmcsemacs.org/t/how-to-re-bind-keys/56][on our
Discourse]].
...
Rmcs uses the [[rmcs-package:display-line-numbers]] package, which is included with Emacs 26+.
...
display-line-numbers-type]] ([[kbd:][<help>]] is [[kbd:][SPC h]] for [[rmcs-package:evil]] users, [[kbd:][C-h]] otherwise).

*** To cycle through different styles of line numbers
Use ~M-x rmcs/toggle-line-numbers~ (bound to [[kbd:][<leader> t l]] by default) to cycle
...
By disabling the [[rmcs-module::editor evil]] module ([[id:01cffea4-3329-45e2-a892-95a384ab2338][how to toggle modules]]).

Read the "[[id:f3925da6-5f0b-4d11-aa08-7bb58bea1982][Removing evil-mode]]" section in the module's documentation for precise
instructions.

** Know when to run ~$ rmcs sync~?
:PROPERTIES:
:ID:       594d2505-d3cb-4061-ab76-06e7c8a4e0b8
:END:
As a rule of thumb, run ~$ rmcs sync~ whenever you:

- Update Rmcs with ~$ git pull~ instead of ~$ rmcs upgrade~,
- Change your ~rmcs!~ block in =$RMCSDIR/init.el=,
- Change =autoload.el= or =autoload/*.el= files in any module (or =$RMCSDIR=),
- Change the =packages.el= file in any module (or =$RMCSDIR=).
- Install an Emacs package or dependency outside of Emacs (i.e. through your OS
  package manager).

If anything is misbehaving, it's a good idea to run ~$ rmcs sync~ first, to rule
out any issues with missing packages or autoloads.

This command is never needed for changes to =$RMCSDIR/config.el=.

** Suppress confirmation prompts when executing a rmcs command?
~-!~ or ~--force~ are the universal "suppress all prompts" switch for most
=rmcs= commands.

** Copy or sync my config to another system?
*Short answer:* it is safe to sync =$RMCSDIR= across systems, but not
=$EMACSDIR=. Once moved, use ~$ rmcs sync && rmcs build~ to ensure everything is
set up correctly.
...
should be solved by running ~$ rmcs sync && rmcs build~ on the other end, once
moved.

** Start over, in case something went terribly wrong?
Delete =$EMACSDIR/.local/straight= and run ~$ rmcs sync~.

** Restore the s and S keys to their default vim behavior ([[rmcs-ref:][#1307]])
[[kbd:][s]] and [[kbd:][S]] have been intentionally replaced with the [[rmcs-package:evil-snipe]] plugin, which
...
#+begin_src emacs-lisp
;; in $RMCSDIR/config.el
(remove-hook 'rmcs-first-input-hook #'evil-snipe-mode)
#+end_src
...
:END:
For problems that come up especially often. You'll find more [[https://discourse.rmcsemacs.org][on our Discourse]]
and [[https://github.com/rmcsemacs/rmcsemacs/issues][Github issue tracker]]. Consult [[id:14040b81-3dd3-422e-abce-245f4c03d1b4][our troubleshooting guide]] for help debugging
issues.

** Rmcs and/or Emacs is slow
:PROPERTIES:
:ID:       643acc75-9717-4739-9c52-3947491e827e
:END:
The answer changes from version to version (of Emacs), and is often updated with
new information, so [[https://discourse.rmcsemacs.org/t/why-is-emacs-rmcs-slow/83][this is answered on Discourse]] instead.

** Rmcs starts up with a vanilla splash screen
The most common cause for this is a =~/.emacs= file. If it exists, Emacs will
read this file instead of the =~/.emacs.d= directory, ignoring Rmcs altogether.

If this isn't the case, run ~$ rmcs doctor~. It can detect a variety of common
...
- Emacs can't find your private rmcs config (in =~/.rmcs.d= or
  =~/.config/rmcs=). Make sure only one of these two folders exist, and that it
  has an =init.el= file with a ~rmcs!~ block. Running ~$ rmcs install~ will
  create these files and directories for you.

- An error occurred while starting up Rmcs. Use [[kbd:][C-h e]] to inspect Emacs' log.
...
- You have disabled the [[rmcs-module::ui rmcs-dashboard]] module. Read about
  [[id:5e267107-81fa-45b4-8ff3-26d4b98e508e][what Rmcs modules are]] and [[id:01cffea4-3329-45e2-a892-95a384ab2338][how to
  toggle them]].

If you're still stuck, run ~$ rmcs doctor~. It can detect a variety of common
issues and may give you some clues as to what is wrong.

** Rmcs fails to find executables (or inherit my shell's =$PATH=)
The three most common causes for =$PATH= issues in Rmcs are:
...
issues #1 and #3: generate an envvar file by running ~$ rmcs env~. This scrapes
your shell environment into a file that is loaded when Rmcs Emacs starts up. Run
~$ rmcs help env~ for details on how this works.
...
[[https://rmcsemacs.org/discord][our Discord]] or [[https://discourse.rmcsemacs.org][Discourse]].

** Changes to my config aren't taking effect
1. Make sure you don't have both =~/.rmcs.d= and =~/.config/rmcs= directories.
   Rmcs will ignore the first if the second exists.
2. Remember to run ~$ rmcs sync~ after making [[id:594d2505-d3cb-4061-ab76-06e7c8a4e0b8][certain changes]] to your config.
   Run ~$ rmcs help sync~ to know exactly when you should use it.
...
If none of these solve your issue, try ~$ rmcs doctor~. It will detect a variety
...
- Some fonts cause Emacs to crash when they lack support for a particular glyph
  (typically symbols). Try changing your font by changing ~rmcs-font~ or
  ~rmcs-symbol-font~.

- Ligatures can cause Emacs to crash. Try a different [[rmcs-module::ui ligatures +fira][ligature font]] or disable
  the [[rmcs-module::ui ligatures]] module altogether.

- On some systems (particularly MacOS), manipulating the fringes or window
  margins can cause Emacs to crash. This is most prominent in Rmcs's Dashboard
  ...
  Or disable the [[rmcs-module::ui rmcs-dashboard]] and [[rmcs-module::tools magit]] modules (see [[rmcs-ref:][#1170]]).
...
[[https://github.com/rmcsemacs/rmcsemacs/blob/4eeb3c7a19c324f5a7839a2e3edb03fc87d23034/core/core-ui.el#L97-L116][Rmcs highlights non-standard indentation]]. i.e. Indentation that doesn't match
the indent style you've set for that file. Spaces are Rmcs's default style for
...
3. Use [[https://editorconfig.org/][editorconfig]] to configure code style on a per-project basis. If you
   enable Rmcs's [[rmcs-module::tools editorconfig]] module, Rmcs will recognize
   =.editorconfigrc= files.

4. Or trust in [[rmcs-package:dtrt-indent]]; a plugin Rmcs uses to analyze and detect indentation
...
- =tar= and/or =gzip= aren't in your =$PATH=, somehow. Once you've corrected
  your shell config, run ~$ rmcs env~ to regenerate your envvar file (containing
  the =$PATH= Rmcs will see), then restart Emacs.
...
[[rmcs-user:hlissner][meatball]] and a handful of busy volunteers -- so there may be delays processing
...
progress, feel free to ping him in-thread or [[https://rmcsemacs.org/discord][on Discord]] (in the #contributing
channel).
...
Since Rmcs's "support team" only consists of one overworked maintainer and a
...
- Behavior that can't be reproduced in [[id:04f91253-a92a-4125-a576-44de226582bb][vanilla Rmcs]].
...
- A post asking how to configure or use Emacs to achieve some effect (our issue
  tracker is for bug reports, not support; ask on [[https://rmcsemacs.org/discord][Discord]] or [[https://discourse.rmcsemacs.org][Discourse]] instead).
...
** How do I create and maintain a PR branch for Rmcs Emacs?
...
and directly translates to more hours on my Rmcs, Emacs-related, or open-source
capers.
...
- *Module maintainers* look after one or more Rmcs modules. They become my
...
Folks that depend on Rmcs for their work or businesses would benefit from
...
Feel free to DM me (Henrik#0666) on [[https://rmcsemacs.org/discord][Discord]], ask on [[https://discourse.rmcsemacs.org][Discourse]], or email me at
contact@henrik.io.
...
:END:
For questions regarding Rmcs's code design, defaults, or conventions.

** Why does Rmcs sharp-quote function symbols?
...
** TODO How does Rmcs Emacs start up so quickly?
#+begin_quote
 󱌣 *This post is a work in progress!* However, there's a post on our Discourse
    that outlines [[https://discourse.rmcsemacs.org/t/how-does-rmcs-start-up-so-quickly/163/1][some of our older techniques]].
#+end_quote

** TODO How does Rmcs Emacs improve runtime performance?
#+begin_quote
 󱌣 *This post is a work in progress!*
#+end_quote

** Why does Rmcs not use dash, f, s, or similar libraries?
=subr-x=, =seq=, =map=, =pcase=, and =cl-lib= are all built into Emacs and,
since Emacs 27.1 (the minimum version Rmcs supports), have made Dash and co
(mostly) redundant. One of Rmcs's goals is to prefer native functionality where
...
** Why does Rmcs discourage the use of ~M-x customize~?
...
- The way it applies its settings (through theme variables) defies conventional
  load order, which is troublesome to support in middleware like Rmcs (or user
...
- Without its /unparalleled/ extensibility, I believe Emacs isn't particularly
  ...
  with issues or tweak where =Customize= can't, and Rmcs wants to help you face
...
All that said, I take no steps to disable or cripple =Customize= in Rmcs
...
I believe [[rmcs-package:expand-region]] is redundant with and less precise than evil's text
objects and motions.
...
The [[rmcs-package:expand-region]] way dictates you start at some point and expand/contract until
...
That said, if you aren't convinced, it is trivial to install [[rmcs-package:expand-region]] and
...
** Why ~rmcs env~ instead of ~exec-path-from-shell~?
...
[[rmcs-package:exec-path-from-shell]] was written to mitigate this, by polling the shell at
startup for those environment variables. ~$ rmcs env~ was written as more
reliable (and slightly faster) substitute. Here's why it's better:

1. [[rmcs-package:exec-path-from-shell]] must spawn (at least) one process at startup to scrape
...
2. [[rmcs-package:exec-path-from-shell]] takes a whitelist approach and captures only =$PATH= and
   ...
   ~$ rmcs env~ takes the blacklist approach and captures all of your shell
...
I believe [[rmcs-package:ws-butler]] is less imposing on teammates/project maintainers; it only
