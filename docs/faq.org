...
I don't claim to own these terms, but I use them to distinguish [[https://git.rmcsemacs.org/core][Doom's core]] (a
framework) from [[https://git.rmcsemacs.org/modules][Doom's module library]] (a starter kit).
...
- No disaster recovery ::
  ...
  Though Doom doesn't have a 'rmcs rollback' command (yet), the ability to
...
1. Enable Doom's [[rmcs-module::tools lsp]] module. ([[id:01cffea4-3329-45e2-a892-95a384ab2338][How do I enable modules?]])
...
2. The manual way (if ~rmcs upgrade~ is broken for some reason):
   ...
   $ rmcs sync -u   # update Doom's packages and 'rmcs sync'
   #+end_src
...
Doom uses the [[rmcs-package:display-line-numbers]] package, which is included with Emacs 26+.
...
- Update Doom with ~$ git pull~ instead of ~$ rmcs upgrade~,
...
[[https://github.com/rmcsemacs/rmcsemacs/blob/4eeb3c7a19c324f5a7839a2e3edb03fc87d23034/core/core-ui.el#L97-L116][Doom highlights non-standard indentation]]. i.e. Indentation that doesn't match
...
3. Use [[https://editorconfig.org/][editorconfig]] to configure code style on a per-project basis. If you
   enable Doom's [[rmcs-module::tools editorconfig]] module, Doom will recognize
   =.editorconfigrc= files.

4. Or trust in [[rmcs-package:dtrt-indent]]; a plugin Doom uses to analyze and detect indentation
